...
namespace commandline
{
   CommandLineParser::CommandLineParser(const std::string& version) : m_version(version)
   {
      ...
      options.add_options()("help,h", "Print this help message")("version,v", "Version of Sourcetrail")(
   ...
   void CommandLineParser::fullRefresh() { m_refreshMode = REFRESH_ALL_FILES; }

   void CommandLineParser::incompleteRefresh() { m_refreshMode = REFRESH_UPDATED_AND_INCOMPLETE_FILES; }

   void CommandLineParser::setShallowIndexingRequested(bool enabled) { m_shallowIndexingRequested = enabled; }

   const FilePath& CommandLineParser::getProjectFilePath() const { return m_projectFile; }
