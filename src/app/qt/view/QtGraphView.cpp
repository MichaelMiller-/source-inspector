...
QtGraphView::QtGraphView(ViewLayout* viewLayout)
    : GraphView(viewLayout), m_focusHandler(this), m_centerActiveNode(false), m_scrollToTop(false),
      m_restoreScroll(false), m_isIndexedList(false)
{
...
bool QtGraphView::isVisible() const { return QtViewWidgetWrapper::getWidgetOfView(this)->isVisible(); }
                               ...
                               const std::vector<std::shared_ptr<DummyEdge>>& edges, const GraphParams params)
{
   m_onQtThread([=]() {
      ...
      const auto o =
         GraphViewStyle::alignOnRaster(math::Vec2i(static_cast<int>(center.x()), static_cast<int>(center.y())));
...
math::Vec2i QtGraphView::getViewSize() const
{
   QtGraphicsView* view = getView();

   const float zoomFactor = view->getZoomFactor();
   return {static_cast<int>((view->width() - 50) / zoomFactor), static_cast<int>((view->height() - 100) / zoomFactor)};
}
...
void QtGraphView::scrollToValues(int xValue, int yValue)
{
   ...
   m_scrollValues = math::Vec2i(xValue, yValue);
}
...
bool QtGraphView::hasNavigationFocus() const { return m_hasFocus; }
...
QtGraphNode* QtGraphView::getActiveNode() const { return m_oldActiveNode; }

void QtGraphView::ensureNodeVisible(QtGraphNode* node)
{
   QtGraphicsView* view = getView();

   auto r = node->getBoundingRect();
...
void QtGraphView::clickedCustomTrail() { MessageCustomTrailShow().dispatch(); }

void QtGraphView::clickedBackwardTrail() { activateTrail(false); }

void QtGraphView::clickedForwardTrail() { activateTrail(true); }
...
void QtGraphView::doResize() { getView()->setSceneRect(getSceneRect(m_oldNodes)); }

QtGraphNode* QtGraphView::createNodeRecursive(QGraphicsView* view, QtGraphNode* parentNode, const DummyNode* node,
                                              bool multipleActive, bool interactive)
{
   if (!node->visible) {
      return nullptr;
   }
   ...
   if (node->isGraphNode()) {
      newNode = new QtGraphNodeData(&m_focusHandler, node->data, node->name, node->childVisible,
                                    node->getQualifierNode() != nullptr, interactive);
      ...
      newNode = new QtGraphNodeBundle(&m_focusHandler, node->tokenId, node->getBundledNodeCount(),
                                      node->bundledNodeType, node->name, interactive);
      ...
      newNode = new QtGraphNodeGroup(&m_focusHandler, node->tokenId, node->name, node->groupType,
...
QtGraphEdge* QtGraphView::createEdge(QGraphicsView* view, const DummyEdge* edge, std::set<Id>* visibleEdgeIds,
                                     Graph::TrailMode trailMode, QPointF pathOffset, bool useBezier, bool interactive)
{
   if (!edge->visible) {
      return nullptr;
   }
   ...
   if (owner != nullptr && target != nullptr) {
      QtGraphEdge* qtEdge = new QtGraphEdge(&m_focusHandler, owner, target, edge->data, edge->getWeight(), edge->active,
                                            interactive, edge->layoutHorizontal, edge->getDirection());

      if (trailMode != Graph::TRAIL_NONE) {
         std::vector<math::Vec4i> path = edge->path;
...
QtGraphEdge* QtGraphView::createBundledEdgesEdge(QGraphicsView* view, const DummyEdge* edge,
                                                 std::set<Id>* visibleEdgeIds, bool interactive)
{
   if (!edge->visible) {
      return nullptr;
   }
...
void QtGraphView::centerNode(QtGraphNode* node)
{
   ...
   math::Vec2i pos = node->getPosition();
   math::Vec2i size = node->getSize();
   ...
   for (std::list<QtGraphNode*>::iterator it = newSubNodes.begin(); it != newSubNodes.end(); it++) {
      ...
      for (std::list<QtGraphNode*>::iterator it2 = oldSubNodes.begin(); it2 != oldSubNodes.end(); it2++) {
         if (((*it)->isDataNode() && (*it2)->isDataNode() && (*it)->getTokenId() == (*it2)->getTokenId()) ||
            ...
            compareNodesRecursive((*it)->getSubNodes(), (*it2)->getSubNodes(), appearingNodes, vanishingNodes,
                                  remainingNodes);
