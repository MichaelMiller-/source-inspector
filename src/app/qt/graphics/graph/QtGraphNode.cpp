...
void QtGraphNode::blendIn() { setOpacity(1.0f); }

void QtGraphNode::blendOut() { setOpacity(0.0f); }

void QtGraphNode::showNode() { this->show(); }

void QtGraphNode::hideNode() { this->hide(); }
...
QtGraphNode* QtGraphNode::getParent() const { return m_parentNode; }
...
QtGraphNode* QtGraphNode::getLastNonGroupParent() const { return getLastParent(true); }
...
const std::list<QtGraphNode*>& QtGraphNode::getSubNodes() const { return m_subNodes; }

math::Vec2i QtGraphNode::getPosition() const
{
   return math::Vec2i(static_cast<int>(this->scenePos().x()), static_cast<int>(this->scenePos().y()));
}

bool QtGraphNode::setPosition(const math::Vec2i& position)
{
   math::Vec2i currentPosition = getPosition();
   math::Vec2i offset = position - currentPosition;

   if (offset.x != 0 || offset.y != 0) {
      this->moveBy(offset.x, offset.y);
      setColumnSize(math::Vec2i());
...
const math::Vec2i& QtGraphNode::getSize() const { return m_size; }

void QtGraphNode::setSize(const math::Vec2i& size)
{
   m_size = size;
...
const math::Vec2i& QtGraphNode::getColumnSize() const { return m_columnSize; }

void QtGraphNode::setColumnSize(const math::Vec2i& size) { m_columnSize = size; }

QSize QtGraphNode::size() const { return QSize(m_size.x, m_size.y); }

void QtGraphNode::setSize(QSize size) { setSize(math::Vec2i(size.width(), size.height())); }

math::Vec4i QtGraphNode::getBoundingRect() const
{
   math::Vec2i pos = getPosition();
   math::Vec2i size = getSize();
   return math::Vec4i(pos.x, pos.y, pos.x + size.x, pos.y + size.y);
}

void QtGraphNode::addOutEdge(QtGraphEdge* edge) { m_outEdges.push_back(edge); }

void QtGraphNode::addInEdge(QtGraphEdge* edge) { m_inEdges.push_back(edge); }

size_t QtGraphNode::getOutEdgeCount() const { return m_outEdges.size(); }

size_t QtGraphNode::getInEdgeCount() const { return m_inEdges.size(); }

bool QtGraphNode::getIsActive() const { return m_isActive; }

void QtGraphNode::setIsActive(bool isActive) { m_isActive = isActive; }

void QtGraphNode::setMultipleActive(bool multipleActive) { m_multipleActive = multipleActive; }
...
bool QtGraphNode::isFocusable() const { return m_isInteractive && (isDataNode() || isGroupNode() || isBundleNode()); }

std::wstring QtGraphNode::getName() const { return m_text->text().toStdWString(); }

void QtGraphNode::setName(const std::wstring& name) { m_text->setText(QString::fromStdWString(name)); }
...
bool QtGraphNode::getIsFocused() const { return m_isFocused; }
...
void QtGraphNode::setActiveMatch(bool active) { m_isActiveMatch = active; }

bool QtGraphNode::isDataNode() const { return false; }

bool QtGraphNode::isAccessNode() const { return false; }

bool QtGraphNode::isExpandToggleNode() const { return false; }

bool QtGraphNode::isBundleNode() const { return false; }

bool QtGraphNode::isQualifierNode() const { return false; }

bool QtGraphNode::isTextNode() const { return false; }

bool QtGraphNode::isGroupNode() const { return false; }

Id QtGraphNode::getTokenId() const { return 0; }
...
void QtGraphNode::moved(const math::Vec2i& oldPosition)
{
...
void QtGraphNode::setStyle(const GraphViewStyle::NodeStyle& style)
{
   ...
   m_text->setPos(static_cast<qreal>(style.iconOffset.x + style.iconSize + style.textOffset.x),
   ...
   if (m_matchLength) {
      ...
      m_matchRect->setRect(
         static_cast<qreal>(style.iconOffset.x + style.iconSize + style.textOffset.x + m_matchPos * charWidth),
         static_cast<qreal>(style.textOffset.y), static_cast<qreal>(m_matchLength * charWidth),
