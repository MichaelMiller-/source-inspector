...
QtGraphEdge::QtGraphEdge(GraphFocusHandler* focusHandler, QtGraphNode* owner, QtGraphNode* target, const Edge* data,
                         size_t weight, bool isActive, bool isInteractive, bool horizontal,
                         TokenComponentBundledEdges::Direction direction)
    : m_focusHandler(focusHandler), m_data(data), m_owner(owner), m_target(target), m_isActive(isActive),
      m_isHorizontal(horizontal), m_weight(weight), m_direction(direction), m_isInteractive(isInteractive)
{
...
const Edge* QtGraphEdge::getData() const { return m_data; }

QtGraphNode* QtGraphEdge::getOwner() { return m_owner; }

QtGraphNode* QtGraphEdge::getTarget() { return m_target; }
...
void QtGraphEdge::updateLine()
{
   ...
   math::Vec4i ownerRect = owner->getBoundingRect();
   math::Vec4i targetRect = target->getBoundingRect();

   math::Vec4i ownerParentRect;
   math::Vec4i targetParentRect;
   ...
   if (m_useBezier) {
      ...
      for (const auto& rect : m_path) {
         ...
         if (route == QtLineItemBase::ROUTE_HORIZONTAL) {
            line->updateLine(math::Vec2i(rect.x(), (rect.y() + rect.w()) / 2), math::Vec2i(rect.z(), (rect.y() + rect.w()) / 2),
                             style);
         } else {
            line->updateLine(math::Vec2i((rect.x() + rect.z()) / 2, rect.y()), math::Vec2i((rect.x() + rect.z()) / 2, rect.w()),
                             style);
         }
      ...
      const math::Vec2i* ownerColumnSize;
      const math::Vec2i* targetColumnSize;
...
bool QtGraphEdge::getIsActive() const { return m_isActive; }
...
bool QtGraphEdge::isFocusable() const { return m_isInteractive; }

void QtGraphEdge::onClick()
{
   if (isExpandable()) {
      QtGraphNode* node = (m_direction == TokenComponentBundledEdges::DIRECTION_BACKWARD ? m_owner : m_target);
      ...
      MessageActivateTrailEdge({getData()->getId()}, getData()->getType(), getData()->getFrom()->getNameHierarchy(),
      ...
      MessageActivateEdge msg(getData()->getId(),
                              componentInheritance ? Edge::EDGE_BUNDLED_EDGES : getData()->getType(),
                              getData()->getFrom()->getNameHierarchy(), getData()->getTo()->getNameHierarchy());

      if (getData()->getType() == Edge::EDGE_BUNDLED_EDGES) {
         msg.bundledEdgesIds =
            utility::toVector<Id>(getData()->getComponent<TokenComponentBundledEdges>()->getBundledEdgesIds());
...
void QtGraphEdge::coFocusIn()
{
   if (!m_isCoFocused) {
      ...
      if (s_focusedEdge == this) {
         ...
         if (type == Edge::EDGE_BUNDLED_EDGES && m_direction == TokenComponentBundledEdges::DIRECTION_NONE) {
...
{
   m_mousePos = math::Vec2i(static_cast<int>(event->scenePos().x()), static_cast<int>(event->scenePos().y()));
   m_mouseMoved = false;
}

void QtGraphEdge::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
   auto mousePos = math::Vec2i(static_cast<int>(event->scenePos().x()), static_cast<int>(event->scenePos().y()));
...
bool QtGraphEdge::isHorizontal() const { return m_isHorizontal; }

bool QtGraphEdge::isExpandable() const { return !getData() || m_owner->isGroupNode() || m_target->isGroupNode(); }

bool QtGraphEdge::isTrailEdge() const { return m_isTrailEdge; }

void QtGraphEdge::setIsTrailEdge(const std::vector<math::Vec4i>& path, bool horizontal)
{
   m_path = path;
   m_isTrailEdge = true;
   m_useBezier = true;
...
bool QtGraphEdge::isBezierEdge() const { return m_useBezier; }
...
void QtGraphEdge::clearPath() { m_path.clear(); }

bool QtGraphEdge::isAmbiguous() const { return m_data && m_data->getComponent<TokenComponentIsAmbiguous>(); }
