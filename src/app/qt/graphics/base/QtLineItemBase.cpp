...
QtLineItemBase::QtLineItemBase(QGraphicsItem* parent)
    : QGraphicsLineItem(parent), m_showArrow(true), m_onFront(false), m_onBack(false), m_earlyBend(false),
      m_route(ROUTE_ANY)
{
...
void QtLineItemBase::updateLine(const math::Vec4i& ownerRect, const math::Vec4i& targetRect,
                                const math::Vec4i& ownerParentRect, const math::Vec4i& targetParentRect,
                                const GraphViewStyle::EdgeStyle& style, size_t weight, bool showArrow)
{
...
void QtLineItemBase::setRoute(Route route) { m_route = route; }

void QtLineItemBase::setOnFront(bool front) { m_onFront = front; }

void QtLineItemBase::setOnBack(bool back) { m_onBack = back; }

void QtLineItemBase::setEarlyBend(bool earlyBend) { m_earlyBend = earlyBend; }
...
QPolygon QtLineItemBase::getPath() const
{
   ...
   const math::Vec4i& oR = m_ownerRect;
   const math::Vec4i& oPR = m_ownerParentRect;
   const math::Vec4i& tR = m_targetRect;
   const math::Vec4i& tPR = m_targetParentRect;

   const math::Vec2i& oOff = m_style.originOffset;
   const math::Vec2i& tOff = m_style.targetOffset;
   ...
   if (it != io) {
      ...
      } else if ((it == 3 && b.x() < c.x()) || (io == 3 && b.x() > c.x()) || (it == 0 && b.y() < c.y()) ||
                 (io == 0 && b.y() > c.y())) {
...
QRectF QtLineItemBase::getArrowBoundingRect(const QPolygon& poly) const
{
   ...
   QRectF rect(poly.at(0).x(), poly.at(0).y(), (dir % 2 == 1 ? m_style.arrowLength : m_style.arrowWidth),
...
void QtLineItemBase::getPivotPoints(math::Vec2f* p, const math::Vec4i& in, const math::Vec4i& out, int offset,
                                    bool target) const
{
   float f = 1 / 2.f;

   p[0] = math::Vec2f(static_cast<float>(in.x + (in.z - in.x) * f + offset), static_cast<float>(out.y));
   p[2] = math::Vec2f(static_cast<float>(in.x + (in.z - in.x) * f + offset), static_cast<float>(out.w));

   p[1] = math::Vec2f(static_cast<float>(out.z), static_cast<float>(in.y + (in.w - in.y) * f + offset));
   p[3] = math::Vec2f(static_cast<float>(out.x), static_cast<float>(in.y + (in.w - in.y) * f + offset));
}
