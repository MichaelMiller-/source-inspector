...
void QtProjectWizardContentExtensions::populate(QGridLayout* layout, int& row)
{
   ...
   if (std::dynamic_pointer_cast<SourceGroupSettingsWithSourceExtensionsC>(m_settings) ||
      ...
      cxxAddition =
         QStringLiteral(" Files with these extensions will serve as entry points for the indexer. Headers that "
...
void QtProjectWizardContentExtensions::load() { m_listBox->setStrings(m_settings->getSourceExtensions()); }

void QtProjectWizardContentExtensions::save() { m_settings->setSourceExtensions(m_listBox->getStrings()); }
