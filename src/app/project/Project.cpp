...
Project::Project(std::shared_ptr<ProjectSettings> settings, StorageCache* storageCache, const std::string& appUUID,
                 bool hasGUI)
    : m_settings(settings), m_storageCache(storageCache), m_state(PROJECT_STATE_NOT_LOADED),
      m_refreshStage(RefreshStageType::NONE), m_appUUID(appUUID), m_hasGUI(hasGUI)
{
}

Project::~Project() {}

FilePath Project::getProjectSettingsFilePath() const { return m_settings->getFilePath(); }

std::string Project::getDescription() const { return m_settings->getDescription(); }
...
bool Project::isIndexing() const { return m_refreshStage == RefreshStageType::INDEXING; }
...
void Project::load(std::shared_ptr<DialogView> dialogView)
{
...

   {
      if (tempDbPath.exists()) {
         if (dbPath.exists()) {
            ...
            LOG_INFO("Switching to temporary indexing data because no other persistent data was "
                     "found");
   ...
   if (canLoad) {
      ...
      switch (m_state) {
      case PROJECT_STATE_NEEDS_MIGRATION:
         MessageStatus(L"Project could not be loaded and needs to be re-indexed after automatic migration "
                       L"to latest "
                       "version.",
                       false, false)
            .dispatch();
         break;
      case PROJECT_STATE_EMPTY:
         MessageStatus(L"Project could not load any symbols because the index database is empty. Please "
                       L"re-index the "
                       "project.",
                       false, false)
            .dispatch();
      ...
      case PROJECT_STATE_OUTVERSIONED:
         MessageStatus(L"Project could not be loaded because the indexed data format is incompatible to "
                       L"the current "
...
void Project::refresh(std::shared_ptr<DialogView> dialogView, RefreshMode refreshMode, bool shallowIndexingRequested)
{
   ...
   case PROJECT_STATE_OUTDATED:
      question = L"The project file was changed after the last indexing. The project needs to get fully "
                 L"reindexed to "
   ...
   case PROJECT_STATE_OUTVERSIONED:
      question = L"This project was indexed with a different version of Sourcetrail. It needs to be "
   ...
   case PROJECT_STATE_NEEDS_MIGRATION:
      question = L"This project was created with a different version and uses an old project file "
                 L"format. "
   ...
   case PROJECT_STATE_DB_CORRUPTED:
      question = L"There was a problem loading the index of this project. The project needs to get "
                 L"fully reindexed. "
   ...
   if (m_hasGUI) {
      ...
      dialogView->startIndexingDialog(
         this, enabledModes, refreshMode, allowsShallowIndexing, useShallowIndexing,
...
void Project::buildIndex(RefreshInfo info, std::shared_ptr<DialogView> dialogView)
{
   ...
   if (info.mode != REFRESH_ALL_FILES && (info.filesToClear.size() || info.nonIndexedFilesToClear.size())) {
      taskSequential->addTask(std::make_shared<TaskCleanStorage>(
         tempStorage, dialogView, utility::toVector(utility::concat(info.filesToClear, info.nonIndexedFilesToClear)),
   ...
   if (!indexerCommandProvider->empty()) {
      const int adjustedIndexerThreadCount =
         std::min<int>(indexerThreadCount, static_cast<int>(indexerCommandProvider->size()));
      ...
      taskParallelIndexing->addChildTasks(std::make_shared<TaskGroupSequence>()->addChildTasks(
         ...
         std::make_shared<TaskBuildIndex>(adjustedIndexerThreadCount, storageProvider, dialogView, m_appUUID,
                                          multiProcess)));
      ...
      taskParallelIndexing->addTask(std::make_shared<TaskGroupSequence>()->addChildTasks(
         ...
         std::make_shared<TaskDecoratorRepeat>(TaskDecoratorRepeat::CONDITION_WHILE_SUCCESS, Task::STATE_SUCCESS, 250)
            ->addChildTask(std::make_shared<TaskGroupSelector>()->addChildTasks(
               ...
               std::make_shared<TaskReturnSuccessIf<bool>>("indexer_threads_stopped",
                                                           TaskReturnSuccessIf<bool>::CONDITION_EQUALS, false)))));
   ...
   if (!customIndexerCommandProvider->empty()) {
      const int adjustedIndexerThreadCount =
         std::min<int>(indexerThreadCount, static_cast<int>(customIndexerCommandProvider->size()));

      taskSequential->addTask(std::make_shared<TaskExecuteCustomCommands>(
         std::move(customIndexerCommandProvider), tempStorage, dialogView, adjustedIndexerThreadCount,
      ...
      std::make_shared<TaskGroupSequence>()->addChildTasks(
         std::make_shared<TaskFindKeyOnBlackboard>("keep_database"), std::make_shared<TaskLambda>([dialogView, this]() {
            Task::dispatch(TabId::app(),
                           std::make_shared<TaskLambda>([dialogView, this]() { swapToTempStorage(dialogView); }));
         })),
      std::make_shared<TaskGroupSequence>()->addChildTasks(
...
bool Project::swapToTempStorageFile(const FilePath& indexDbFilePath, const FilePath& tempIndexDbFilePath,
                                    std::shared_ptr<DialogView> dialogView)
{
   try {
      ...
      if (m_hasGUI) {
         dialogView->confirm(L"<p>The old index database file of this project seems to be used by a different "
