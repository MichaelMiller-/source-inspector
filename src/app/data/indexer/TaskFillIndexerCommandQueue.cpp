...
TaskFillIndexerCommandsQueue::TaskFillIndexerCommandsQueue(
   const std::string& appUUID, std::unique_ptr<IndexerCommandProvider> indexerCommandProvider, size_t maximumQueueSize)
    : m_indexerCommandProvider(std::move(indexerCommandProvider)), m_indexerCommandManager(appUUID, 0, true),
...
void TaskFillIndexerCommandsQueue::doReset(std::shared_ptr<Blackboard> blackboard) { m_interrupted = false; }

void TaskFillIndexerCommandsQueue::terminate() { m_interrupted = true; }
