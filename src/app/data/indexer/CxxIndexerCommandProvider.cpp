...
void CxxIndexerCommandProvider::addCommand(const std::shared_ptr<IndexerCommandCxx>& command)
{
   ...
   {
      for (const auto& indexedPath : command->getIndexedPaths()) {
         auto it = m_indexedPathsToIds.find(indexedPath);
   ...
   {
      for (const auto& excludeFilter : command->getExcludeFilters()) {
         auto it = m_excludeFiltersToIds.find(excludeFilter.wstr());
   ...
   {
      for (const auto& includeFilter : command->getIncludeFilters()) {
         auto it = m_includeFiltersToIds.find(includeFilter.wstr());
...

   {
      ...
      auto it = m_workingDirectoriesToIds.find(workingDirectory);
...
std::shared_ptr<IndexerCommand> CxxIndexerCommandProvider::consumeCommand()
{
   if (!m_commands.empty()) {
      ...
      if (it->second) {
         auto command = representationToCommand(it->first, it->second);
         m_commands.erase(it);
         return command;
      }
   }
...
std::vector<std::shared_ptr<IndexerCommand>> CxxIndexerCommandProvider::consumeAllCommands()
{
   ...
   for (std::map<FilePath, std::shared_ptr<CommandRepresentation>>::const_iterator it = m_commands.begin();
        it != m_commands.end(); it++) {
...
void CxxIndexerCommandProvider::clear() { m_commands.clear(); }

size_t CxxIndexerCommandProvider::size() const { return m_commands.size(); }
...
Id CxxIndexerCommandProvider::getId() { return m_nextId++; }

std::shared_ptr<IndexerCommandCxx>
CxxIndexerCommandProvider::representationToCommand(const FilePath& sourceFilePath,
                                                   std::shared_ptr<CommandRepresentation> representation)
{
   std::set<FilePath> indexedPaths;
   for (auto id : representation->m_indexedPathIds) {
      indexedPaths.insert(m_idsToIndexedPaths[id]);
   }

   std::set<FilePathFilter> excludeFilters;
   for (auto id : representation->m_excludeFilterIds) {
      excludeFilters.insert(FilePathFilter(m_idsToExcludeFilters[id]));
   }

   std::set<FilePathFilter> includeFilters;
   for (auto id : representation->m_includeFilterIds) {
   ...
   for (auto id : representation->m_compilerFlagIds) {
      compilerFlags.push_back(m_idsToCompilerFlags[id]);
   }

   return std::make_shared<IndexerCommandCxx>(sourceFilePath, indexedPaths, excludeFilters, includeFilters,
                                              workingDirectory, compilerFlags);
}
