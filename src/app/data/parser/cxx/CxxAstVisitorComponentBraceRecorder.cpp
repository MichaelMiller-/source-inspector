	...
	clang::SourceLocation searchStartLoc, clang::SourceLocation searchEndLoc) const
{
	...
	while (true)
	{
		auto token = clang::Lexer::findNextToken(searchStartLoc, sm, opts);
		if (token)
		{
	...
	clang::SourceLocation searchStartLoc, clang::SourceLocation searchEndLoc) const
{
...

	{
		...
		auto token = clang::Lexer::findNextToken(searchEndLoc, sm, opts);
		if (token && token.value().getKind() == clang::tok::r_brace)
		{
			return token.value().getLocation();
		}
	}
