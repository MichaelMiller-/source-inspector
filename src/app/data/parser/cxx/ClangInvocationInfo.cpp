...
namespace
{
   ...
   clang::driver::Driver* newDriver(clang::DiagnosticsEngine* Diagnostics, const char* BinaryName,
                                    clang::IntrusiveRefCntPtr<llvm::vfs::FileSystem> VFS)
   {
      ...
      clang::driver::Driver* CompilerDriver =
         new clang::driver::Driver(BinaryName, llvm::sys::getDefaultTargetTriple(), *Diagnostics, std::move(VFS));
...
ClangInvocationInfo::getClangInvocationString(const clang::tooling::CompilationDatabase* compilationDatabase)
{
   ...
   if (!compilationDatabase->getAllCompileCommands().empty()) {
      ...
      llvm::opt::InputArgList ParsedArgs =
         Opts.ParseArgs(clang::ArrayRef<const char*>(Argv).slice(1), MissingArgIndex, MissingArgCount);
