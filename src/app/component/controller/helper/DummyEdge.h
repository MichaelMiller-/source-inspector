...
{
   DummyEdge()
       : ownerId(0), targetId(0), data(nullptr), visible(false), hidden(false), active(false), layoutHorizontal(true),
         weight(0), direction(TokenComponentBundledEdges::DIRECTION_INVALID)
   {
   }

   DummyEdge(const Id ownerId, const Id targetId, const Edge* data)
       : ownerId(ownerId), targetId(targetId), data(data), visible(false), hidden(false), active(false),
         layoutHorizontal(true), weight(0), direction(TokenComponentBundledEdges::DIRECTION_INVALID)
   {
   }
