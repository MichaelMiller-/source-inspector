...
public:
   ...
   DummyNode(Type type)
       : type(type), visible(false), hidden(false), childVisible(false), tokenId(0), data(nullptr), active(false),
         connected(false), expanded(false), autoExpanded(false), hasParent(true), accessKind(ACCESS_NONE),
         invisibleSubNodeCount(0), bundleId(0), bundledNodeCount(0), bundledNodeType(NODE_SYMBOL),
         qualifierName(NAME_DELIMITER_UNKNOWN), groupType(GroupType::DEFAULT), groupLayout(GroupLayout::LIST),
         interactive(true), fontSizeDiff(5)
   {
   }

   bool isGraphNode() const { return type == DUMMY_DATA; }

   bool isAccessNode() const { return type == DUMMY_ACCESS; }

   bool isExpandToggleNode() const { return type == DUMMY_EXPAND_TOGGLE; }

   bool isBundleNode() const { return type == DUMMY_BUNDLE; }

   bool isQualifierNode() const { return type == DUMMY_QUALIFIER; }

   bool isTextNode() const { return type == DUMMY_TEXT; }

   bool isGroupNode() const { return type == DUMMY_GROUP; }
   ...
   math::Vec4i getActiveSubNodeRect(math::Vec2i pos = {}) const
   {
      pos += position;
      ...
      for (const std::shared_ptr<DummyNode>& node : subNodes) {
         auto rect = node->getActiveSubNodeRect(pos);
         if (rect.w() > 0) {
            return rect;
         }
      }
   ...
   bool getsLayouted() const { return visible && !isExpandToggleNode() && !isQualifierNode(); }
