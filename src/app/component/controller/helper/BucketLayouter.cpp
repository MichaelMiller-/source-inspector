...
int Bucket::getWidth() const { return m_width; }

int Bucket::getHeight() const { return m_height; }
...
const DummyNode::BundledNodesSet& Bucket::getNodes() const { return m_nodes; }

void Bucket::preLayout(math::Vec2i viewSize, bool addVerticalSplit, bool forceVerticalSplit)
{
...
void Bucket::layout(int x, int y, int width, int height)
{
   ...
   math::Vec2i offset = math::Vec2i(x + (width - m_width) / 2, y + (height - m_height) / 2);
...
const std::vector<int> Bucket::getColWidths() const { return m_colWidths; }
...
BucketLayouter::BucketLayouter(math::Vec2i viewSize) : m_viewSize(viewSize), m_i1(0), m_j1(0), m_i2(0), m_j2(0)
{
   m_buckets[0][0] = Bucket(0, 0);
}

void BucketLayouter::createBuckets(std::vector<std::shared_ptr<DummyNode>>& nodes,
...
void BucketLayouter::layoutBuckets(bool addVerticalSplit)
{
   ...
   if (m_activeParentNode) {
      auto rect = m_activeParentNode->getActiveSubNodeRect();
...
std::shared_ptr<DummyNode> BucketLayouter::findTopMostDummyNodeRecursive(std::vector<std::shared_ptr<DummyNode>>& nodes,
                                                                         Id tokenId, std::shared_ptr<DummyNode> top)
{
