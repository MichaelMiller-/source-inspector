...
GraphController::GraphController(StorageAccess* storageAccess) : m_storageAccess(storageAccess), m_useBezierEdges(false)
{
}

Id GraphController::getSchedulerId() const { return Controller::getTabId(); }

void GraphController::handleMessage(MessageActivateErrors* message) { clear(); }

void GraphController::handleMessage(MessageActivateFullTextSearch* message) { clear(); }
...
void GraphController::handleMessage(MessageActivateTokens* message)
{
   ...
   std::shared_ptr<Graph> graph =
      m_storageAccess->getGraphForActiveTokenIds(tokenIds, getExpandedNodeIds(), &isNamespace);
...
void GraphController::handleMessage(MessageActivateTrail* message)
{
   ...
   std::shared_ptr<Graph> graph = m_storageAccess->getGraphForTrail(
      message->originId, message->targetId, message->nodeTypes, message->edgeTypes, message->nodeNonIndexed,
      message->depth, true /* !message->custom || (message->originId && message->targetId) */);
...
void GraphController::handleMessage(MessageFocusIn* message) { getView()->coFocusTokenIds(message->tokenIds); }

void GraphController::handleMessage(MessageFocusOut* message) { getView()->deCoFocusTokenIds(message->tokenIds); }
   ...
   if (dummyNode) {
      ...
      if (message->expand && dummyNode->hasMissingChildNodes()) {
         ...
         childGraph->getNodeById(nodeId)->forEachEdgeOfType(Edge::EDGE_MEMBER,
                                                            [this](Edge* edge) { m_graph->addEdgeAsPlainCopy(edge); });

         Node* node = m_graph->getNodeById(nodeId);
         std::vector<std::shared_ptr<DummyNode>> newDummyNodes =
            createDummyNodeTopDown(node, node->getLastParentNode()->getId());
            ...
            for (size_t i = 0, l = m_dummyEdges.size(); i < l; i++) {
               ...
               if (edge && edge->data && edge->data->isType(Edge::EDGE_BUNDLED_EDGES) &&
                  ...
                  std::shared_ptr<Graph> bundledEdgesGraph =
                     m_storageAccess->getGraphForActiveTokenIds(bundledEdgesIds, std::vector<Id>());

                  bundledEdgesGraph->forEachEdge([this](Edge* e) {
                     if (!e->isType(Edge::EDGE_MEMBER)) {
                        m_dummyEdges.push_back(std::make_shared<DummyEdge>(e->getFrom()->getId(), e->getTo()->getId(),
...
GraphView* GraphController::getView() const { return Controller::getView<GraphView>(); }
...
void GraphController::createDummyGraphAndSetActiveAndVisibility(const std::vector<Id>& tokenIds,
...
void GraphController::updateDummyNodeNamesAndAddQualifiers(const std::vector<std::shared_ptr<DummyNode>>& dummyNodes)
{
      ...
      node->data->forEachNodeRecursive([&bundleInfo](const Node* n) {
         ...
         n->forEachEdgeOfType(~Edge::EDGE_MEMBER, [&bundleInfo, &n](Edge* e) {
            ...
            if (e->isType(Edge::EDGE_BUNDLED_EDGES)) {
               ...
               } else if ((dir == TokenComponentBundledEdges::DIRECTION_FORWARD && e->getFrom() == n) ||
                          (dir == TokenComponentBundledEdges::DIRECTION_BACKWARD && e->getTo() == n)) {
                  bundleInfo->isReferencing = true;
               } else if ((dir == TokenComponentBundledEdges::DIRECTION_FORWARD && e->getTo() == n) ||
   ...
   for (const DummyNode* node : bundledNodes) {
      for (const std::shared_ptr<DummyEdge>& edge : m_dummyEdges) {
         ...
         for (const std::shared_ptr<DummyEdge>& bundleEdge : bundleEdges) {
            if ((owner && bundleEdge->ownerId == edge->targetId) || (target && bundleEdge->ownerId == edge->ownerId)) {
...
std::shared_ptr<DummyNode> GraphController::bundleNodesMatching(std::list<std::shared_ptr<DummyNode>>& nodes,
                                                         ...
                                                         const bool considerInvisibleNodes)
{
   std::shared_ptr<DummyNode> bundleNode = bundleNodesMatching(
      ...
      [&](const DummyNode* node) {
         return (considerInvisibleNodes || node->visible) && node->isGraphNode() && node->data->getType() == type &&
   ...
   if (bundleNode) {
      ...
      if (!bundleInfoTree.children.empty()) {
         std::list<std::shared_ptr<DummyNode>> bundledNodes(bundleNode->bundledNodes.begin(),
                                                            bundleNode->bundledNodes.end());
      ...
      if (dummyNode->isGroupNode()) {
         ...
         if (groupType == GroupType::FILE) {
            ...
            if (qualifierNode) {
               ...
               nodeIdtoParentMap.emplace(dummyNode->tokenId, std::make_pair(qualifierId, qualifierNode->qualifierName));
            }
         }
      }
   }
...
math::Vec4i GraphController::layoutNestingRecursive(DummyNode* node, int relayoutAccessMaxWidth) const
{
   if (!node->visible) {
      return math::Vec4i(0, 0, 0, 0);
   }

   GraphViewStyle::NodeMargins margins;

   if (node->isGraphNode()) {
      margins = GraphViewStyle::getMarginsForDataNode(node->data->getType().getNodeStyle(),
                                                      node->data->getType().hasIcon(), node->childVisible);
      ...
      if (node->bundledNodeType.getKind() != NODE_SYMBOL) {
         margins = GraphViewStyle::getMarginsForDataNode(node->bundledNodeType.getNodeStyle(),
                                                         node->bundledNodeType.hasIcon(), false);
      } else {
   ...
   if (relayoutAccessMaxWidth == -1) {
      ...
      for (const std::shared_ptr<DummyNode>& subNode : node->subNodes) {
         ...
         auto rect = layoutNestingRecursive(subNode.get());
   ...
   if (node->subNodes.size()) {
      if (node->isGroupNode()) {
         math::Vec2i viewSize = getView()->getViewSize();
         ...
         case GroupLayout::SKEWED:
            ListLayouter::layoutSkewed(&node->subNodes, margins.spacingX, margins.spacingY,
   ...
   auto size = ListLayouter::offsetNodes(
...
void GraphController::relayoutGraph(MessageBase* message, GraphView::GraphParams params, bool withCharacterIndex,
...
{
   Id id = ~Id(0) >> 1;
   std::map<Id, math::Vec2i> nodePositions;
   std::shared_ptr<Graph> graph = std::make_shared<Graph>();

   auto addText = [this](std::wstring text, int fontSizeDiff, math::Vec2i position) {
   ...
   auto addNode = [&id, &graph, &nodePositions](NodeKind kind, const std::wstring& name, math::Vec2i position,
                                                DefinitionKind defKind = DEFINITION_EXPLICIT) {
      nodePositions.emplace(++id, position);
      return graph->createNode(id, NodeType(kind), NameHierarchy(name, NAME_DELIMITER_UNKNOWN), defKind);
   };
   ...
   addText(L"Legend", 6, math::Vec2i(0, 0));
   ...
   // Layout
   {
      addText(L"Layout", 3, math::Vec2i(x, y));

      x = 50;
      y = 40;

      Node* base = addNode(NODE_CLASS, L"Base Class", math::Vec2i(x + 220, y + 50));
      Node* main = addNode(NODE_CLASS, L"Class", math::Vec2i(x + 200, y + 130));
      Node* derived = addNode(NODE_CLASS, L"Derived Class", math::Vec2i(x + 210, y + 380));
      Node* user = addNode(NODE_TYPE, L"Referencing Type", math::Vec2i(x - 10, y + 220));
      Node* used = addNode(NODE_TYPE, L"Referenced Type", math::Vec2i(x + 410, y + 220));
      ...
      Node* publicMethod = addNode(NODE_METHOD, L"public method", math::Vec2i());
      Node* privateField = addNode(NODE_FIELD, L"private field", math::Vec2i());
      ...
      Node* func = addNode(NODE_FUNCTION, L"function", math::Vec2i(x + 220, y));
      Node* caller = addNode(NODE_FUNCTION, L"calling function", math::Vec2i(x, y));
      Node* var = addNode(NODE_GLOBAL_VARIABLE, L"accessed variable", math::Vec2i(x + 410, y - 50));
      Node* called = addNode(NODE_FUNCTION, L"called function", math::Vec2i(x + 410, y - 10));
      Node* type = addNode(NODE_TYPE, L"Referenced Type", math::Vec2i(x + 410, y + 30));
   ...
   {
      int i = 0;
      addText(L"Nodes", 3, math::Vec2i(x, y));

      addNode(NODE_FILE, L"File", math::Vec2i(x, y + dy * ++i));
      addNode(NODE_FILE, L"Non-Indexed File", math::Vec2i(x, y + dy * ++i), DEFINITION_NONE);
      Node* incompleteFile = addNode(NODE_FILE, L"Incomplete File", math::Vec2i(x, y + dy * ++i));
      incompleteFile->addComponent(std::make_shared<TokenComponentFilePath>(FilePath(), false));

      addNode(NODE_MACRO, L"Macro", math::Vec2i(x, y + dy * ++i));
      addNode(NODE_ANNOTATION, L"Annotation", math::Vec2i(x, y + dy * ++i));

      addNode(NODE_MODULE, L"module", math::Vec2i(x, y + dy * ++i));
      y -= 15;
      addNode(NODE_NAMESPACE, L"namespace", math::Vec2i(x, y + dy * ++i));
      y -= 15;
      addNode(NODE_PACKAGE, L"package", math::Vec2i(x, y + dy * ++i));
      y -= 15;

      addNode(NODE_TYPE, L"Type", math::Vec2i(x, y + dy * ++i));
      addNode(NODE_TYPE, L"Non-indexed Type", math::Vec2i(x, y + dy * ++i), DEFINITION_NONE);

      addNode(NODE_GLOBAL_VARIABLE, L"variable", math::Vec2i(x, y + dy * ++i));
      y -= 15;
      addNode(NODE_GLOBAL_VARIABLE, L"non-indexed variable", math::Vec2i(x, y + dy * ++i), DEFINITION_NONE);
      y -= 15;

      addNode(NODE_FUNCTION, L"function", math::Vec2i(x, y + dy * ++i));
      y -= 15;
      addNode(NODE_FUNCTION, L"non-indexed function", math::Vec2i(x, y + dy * ++i), DEFINITION_NONE);
      y -= 15;

      Node* typeNode = addNode(NODE_TYPE, L"Type with Members", math::Vec2i(x, y + dy * ++i));
      Node* publicMethod = addNode(NODE_METHOD, L"public method", math::Vec2i());
      Node* protectedMethod = addNode(NODE_METHOD, L"protected method", math::Vec2i());
      Node* privateMethod = addNode(NODE_METHOD, L"private method", math::Vec2i());
      Node* defaultMethod = addNode(NODE_METHOD, L"default method", math::Vec2i());
      Node* publicField = addNode(NODE_FIELD, L"public field", math::Vec2i());
      Node* protectedField = addNode(NODE_FIELD, L"protected field",math::Vec2i());
      Node* privateField = addNode(NODE_FIELD, L"private field", math::Vec2i());
      Node* defaultField = addNode(NODE_FIELD, L"default field", math::Vec2i());
      ...
      addNode(NODE_CLASS, L"Class", math::Vec2i(x, y + dy * ++i));
      addNode(NODE_INTERFACE, L"Interface", math::Vec2i(x, y + dy * ++i));

      addNode(NODE_STRUCT, L"Struct", math::Vec2i(x, y + dy * ++i));
      addNode(NODE_UNION, L"Union", math::Vec2i(x, y + dy * ++i));

      addNode(NODE_TYPEDEF, L"TypeDef", math::Vec2i(x, y + dy * ++i));
      Node* enumNode = addNode(NODE_ENUM, L"Enum", math::Vec2i(x, y + dy * ++i));
      Node* enumConstantNode = addNode(NODE_ENUM_CONSTANT, L"ENUM_CONSTANT", math::Vec2i());
      addMember(enumNode, enumConstantNode);
      y += 10;
      i += 1;

      Node* genericNode = addNode(NODE_TYPE, L"JavaGenericType<ParameterType>",math::Vec2i(x, y + dy * ++i));
      Node* genericParameterNode = addNode(NODE_TYPE_PARAMETER, L"ParameterType", math::Vec2i());
      ...
      groupNode->position = math::Vec2i(x, y + dy * ++i);
      m_dummyNodes.push_back(groupNode);
      y += 25;
      ...
      bundleNode->visible = true;
      bundleNode->position = math::Vec2i(x, y + dy * ++i);
      m_dummyNodes.push_back(bundleNode);
   }
   ...
   // Edges
   {
      addText(L"Edges", 3, math::Vec2i(x, y));
      int i = 0;

      {
         addText(L"file include", 0, math::Vec2i(x, y + dy * ++i));
         Node* file = addNode(NODE_FILE, L"File", math::Vec2i(x, y + dy * ++i));
         Node* fileB = addNode(NODE_FILE, L"File", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_INCLUDE, file, fileB);
      }

      {
         addText(L"class import", 0, math::Vec2i(x, y + dy * ++i));
         Node* file = addNode(NODE_FILE, L"File", math::Vec2i(x, y + dy * ++i));
         Node* type = addNode(NODE_TYPE, L"Class", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_IMPORT, file, type);
      }

      {
         addText(L"macro use", 0, math::Vec2i(x, y + dy * ++i));
         Node* file = addNode(NODE_FILE, L"File", math::Vec2i(x, y + dy * ++i));
         Node* macro = addNode(NODE_MACRO, L"Macro", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_MACRO_USAGE, file, macro);
      }

      {
         addText(L"annotation use", 0, math::Vec2i(x, y + dy * ++i));
         Node* type = addNode(NODE_TYPE, L"Type", math::Vec2i(x, y + dy * ++i));
         Node* macro = addNode(NODE_ANNOTATION, L"Annotation", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_ANNOTATION_USAGE, type, macro);
      }

      {
         addText(L"bundled edges", 0, math::Vec2i(x, y + dy * ++i));
         Node* typeA = addNode(NODE_TYPE, L"Type A", math::Vec2i(x, y + dy * ++i));
         Node* typeB = addNode(NODE_TYPE, L"Type B", math::Vec2i(x + dx, y + dy * i));
      ...
      {
         addText(L"type use", 0, math::Vec2i(x, y + dy * ++i));
         Node* function = addNode(NODE_FUNCTION, L"function", math::Vec2i(x, y + dy * ++i));
         Node* type = addNode(NODE_TYPE, L"Type", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_TYPE_USAGE, function, type);
      }

      {
         addText(L"function call", 0, math::Vec2i(x, y + dy * ++i));
         Node* function = addNode(NODE_FUNCTION, L"function", math::Vec2i(x, y + dy * ++i));
         Node* functionB = addNode(NODE_FUNCTION, L"function", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_CALL, function, functionB);
      }

      {
         addText(L"variable access", 0, math::Vec2i(x, y + dy * ++i));
         Node* function = addNode(NODE_FUNCTION, L"function", math::Vec2i(x, y + dy * ++i));
         Node* variable = addNode(NODE_GLOBAL_VARIABLE, L"variable", math::Vec2i(x + dx, y + dy * i));
         addEdge(Edge::EDGE_USAGE, function, variable);
      }

      {
         addText(L"class inheritance", 0, math::Vec2i(x, y + dy * ++i));
         Node* base = addNode(NODE_CLASS, L"Base Class", math::Vec2i(x, y + dy * (i + 1)));
         Node* derived = addNode(NODE_CLASS, L"Derived Class", math::Vec2i(x, y + dy * (i + 3)));
         addEdge(Edge::EDGE_INHERITANCE, derived, base);

         Node* base2 = addNode(NODE_CLASS, L"Base Class", math::Vec2i(x + 180, y + dy * (i + 1)));
         Node* derived2 = addNode(NODE_CLASS, L"Derived Derived Class", math::Vec2i(x + 180, y + dy * (i + 3)));
      ...
      {
         addText(L"method override", 0, math::Vec2i(x, y + dy * ++i));
         Node* base = addNode(NODE_CLASS, L"Base Class", math::Vec2i(x, y + dy * ++i));
         i += 3;
         Node* derived = addNode(NODE_CLASS, L"Derived Class", math::Vec2i(x, y + dy * i));
         Node* baseMethod = addNode(NODE_METHOD, L"method", math::Vec2i());
         Node* derivedMethod = addNode(NODE_METHOD, L"method", math::Vec2i());
      ...
      {
         addText(L"template specialization", 0, math::Vec2i(x, y + dy * ++i));
         Node* templateFunctionNode =
            addNode(NODE_FUNCTION, L"template_function<typename ParameterType>", math::Vec2i(x, y + dy * ++i));
         y += 20;
         Node* templateFunctionSpecializationNode =
            addNode(NODE_FUNCTION, L"template_function<ArgumentType>", math::Vec2i(x, y + dy * ++i), DEFINITION_IMPLICIT);
         addEdge(Edge::EDGE_TEMPLATE_SPECIALIZATION, templateFunctionSpecializationNode, templateFunctionNode);

         Node* templateNode = addNode(NODE_TYPE, L"TemplateType<typename ParameterType>", math::Vec2i(x, y + dy * ++i));
         y += 30;
         Node* templateSpecializationNode =
            addNode(NODE_TYPE, L"TemplateType<ArgumentType>", math::Vec2i(x, y + dy * ++i), DEFINITION_IMPLICIT);
         Node* argumentNode = addNode(NODE_TYPE, L"ArgumentType", math::Vec2i(x + 270, y + dy * i));
      ...
      {
         addText(L"template member specialization", 0, math::Vec2i(x, y + dy * ++i));
         Node* templateNode = addNode(NODE_TYPE, L"TemplateType<typename ParameterType>", math::Vec2i(x, y + dy * ++i));
         Node* templateMethodNode = addNode(NODE_METHOD, L"method", math::Vec2i());
         addMember(templateNode, templateMethodNode);

         i += 1;

         Node* templateSpecializationNode =
            addNode(NODE_TYPE, L"TemplateType<ArgumentType>", math::Vec2i(x, y + dy * ++i + 20), DEFINITION_IMPLICIT);
         Node* templateSpecializationMethodNode = addNode(NODE_METHOD, L"method", math::Vec2i(), DEFINITION_IMPLICIT);
