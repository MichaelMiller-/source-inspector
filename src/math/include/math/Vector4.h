...
namespace math
{
   ...
   template <class T>
   Vector4<T>::Vector4()
       : VectorBase<T, 4>(), x(&VectorBase<T, 4>::m_values[m_xIndex]), y(&VectorBase<T, 4>::m_values[m_yIndex]),
         z(&VectorBase<T, 4>::m_values[m_zIndex]), w(&VectorBase<T, 4>::m_values[m_wIndex])
   {
      setValue(m_xIndex, 0);
      setValue(m_yIndex, 0);
      setValue(m_zIndex, 0);
   ...
   Vector4<T>::Vector4(const VectorBase<T, 4>& vector)
       : VectorBase<T, 4>(vector), x(&VectorBase<T, 4>::m_values[m_xIndex]), y(&VectorBase<T, 4>::m_values[m_yIndex]),
         z(&VectorBase<T, 4>::m_values[m_zIndex]), w(&VectorBase<T, 4>::m_values[m_wIndex])
   {
   }
