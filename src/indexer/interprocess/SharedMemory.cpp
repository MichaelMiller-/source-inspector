...
SharedMemory::ScopedAccess::ScopedAccess(SharedMemory* memory)
    : boost::interprocess::scoped_lock<boost::interprocess::named_mutex>(memory->getMutex())
      ...
      ,
      m_memoryName(memory->getMemoryName()), m_minimumMemorySize(memory->getInitialMemorySize())
{
   try {
      m_memory =
      ...
      m_memory = boost::interprocess::managed_shared_memory(boost::interprocess::open_or_create,
