...
bool SourceGroupCxxCdb::prepareIndexing()
{
   ...
   if (!cdbPath.empty() && !cdbPath.exists()) {
      std::wstring error = L"Can't refresh project. The compilation database of the project does not exist "
                           L"anymore: " +
                           cdbPath.wstr();
...
std::set<FilePath> SourceGroupCxxCdb::filterToContainedFilePaths(const std::set<FilePath>& filePaths) const
{
   return SourceGroup::filterToContainedFilePaths(
      filePaths, getAllSourceFilePaths(), utility::toSet(m_settings->getIndexedHeaderPathsExpandedAndAbsolute()),
...
std::set<FilePath>
SourceGroupCxxCdb::getAllSourceFilePaths(std::shared_ptr<clang::tooling::JSONCompilationDatabase> cdb) const
{
   ...
   for (const clang::tooling::CompileCommand& command : cdb->getAllCompileCommands()) {
      ...
      if (info.filesToIndex.find(sourcePath) != info.filesToIndex.end() &&
         ...
         provider->addCommand(std::make_shared<IndexerCommandCxx>(
            sourcePath, utility::concat(indexedHeaderPaths, {sourcePath}), excludeFilters, std::set<FilePathFilter>(),
            FilePath(utility::decodeFromUtf8(command.Directory)), utility::concat(cdbFlags, compilerFlags)));
      }
   }
   ...
   if (m_settings->getUseCompilerFlags()) {
      ...
      if (cdb) {
         ...
         for (const clang::tooling::CompileCommand& command : cdb->getAllCompileCommands()) {
            ...
            if (!sourcePath.isAbsolute()) {
               sourcePath =
                  FilePath(utility::decodeFromUtf8(command.Directory + '/' + command.Filename)).makeCanonical();
               if (!sourcePath.isAbsolute()) {
                  sourcePath = cdbPath.getParentDirectory().getConcatenated(sourcePath).makeCanonical();
               }
            }
...
std::shared_ptr<SourceGroupSettings> SourceGroupCxxCdb::getSourceGroupSettings() { return m_settings; }

std::shared_ptr<const SourceGroupSettings> SourceGroupCxxCdb::getSourceGroupSettings() const { return m_settings; }
