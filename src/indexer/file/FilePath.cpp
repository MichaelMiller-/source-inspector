...
FilePath::FilePath()
    : m_path(std::make_unique<boost::filesystem::path>("")), m_exists(false), m_checkedExists(false),
      m_isDirectory(false), m_checkedIsDirectory(false), m_canonicalized(false)
{
}

FilePath::FilePath(const std::string& filePath)
    : m_path(std::make_unique<boost::filesystem::path>(filePath)), m_exists(false), m_checkedExists(false),
      m_isDirectory(false), m_checkedIsDirectory(false), m_canonicalized(false)
{
}
...
FilePath::FilePath(const FilePath& other)
    : m_path(std::make_unique<boost::filesystem::path>(other.getPath())), m_exists(other.m_exists),
      m_checkedExists(other.m_checkedExists), m_isDirectory(other.m_isDirectory),
      m_checkedIsDirectory(other.m_checkedIsDirectory), m_canonicalized(other.m_canonicalized)
{
}

FilePath::FilePath(FilePath&& other)
    : m_path(std::move(other.m_path)), m_exists(other.m_exists), m_checkedExists(other.m_checkedExists),
      m_isDirectory(other.m_isDirectory), m_checkedIsDirectory(other.m_checkedIsDirectory),
      m_canonicalized(other.m_canonicalized)
{
}

FilePath::FilePath(const std::wstring& filePath, const std::wstring& base)
    : m_path(std::make_unique<boost::filesystem::path>(boost::filesystem::absolute(filePath, base))), m_exists(false),
      m_checkedExists(false), m_isDirectory(false), m_checkedIsDirectory(false), m_canonicalized(false)
{
}
...
bool FilePath::empty() const { return m_path->empty(); }
...
bool FilePath::isAbsolute() const { return m_path->is_absolute(); }
...
FilePath FilePath::getLowerCase() const { return FilePath(utility::toLowerCase(wstr())); }
...
std::string FilePath::str() const { return m_path->generic_string(); }

std::wstring FilePath::wstr() const { return m_path->generic_wstring(); }

std::wstring FilePath::fileName() const { return m_path->filename().generic_wstring(); }

std::wstring FilePath::extension() const { return m_path->extension().generic_wstring(); }

FilePath FilePath::withoutExtension() const { return FilePath(getPath().replace_extension().wstring()); }
...
bool FilePath::operator!=(const FilePath& other) const { return !(*this == other); }

bool FilePath::operator<(const FilePath& other) const { return m_path->compare(other.getPath()) < 0; }
