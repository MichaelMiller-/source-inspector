...
namespace
{
   ...
   void addSourceGroupContents(QtProjectWizardContentGroup* group, std::shared_ptr<SettingsType> settings,
   ...
   template <>
   void
   addSourceGroupContents<SourceGroupSettingsCxxCodeblocks>(QtProjectWizardContentGroup* group,
   ...
   template <>
   void addSourceGroupContents<SourceGroupSettingsJavaMaven>(QtProjectWizardContentGroup* group,
   ...
   template <>
   void addSourceGroupContents<SourceGroupSettingsJavaGradle>(QtProjectWizardContentGroup* group,
   ...
   template <>
   void addSourceGroupContents<SourceGroupSettingsPythonEmpty>(QtProjectWizardContentGroup* group,
   ...
   template <>
   void
   addSourceGroupContents<SourceGroupSettingsCustomCommand>(QtProjectWizardContentGroup* group,
...
QtProjectWizard::QtProjectWizard(QWidget* parent)
    : QtProjectWizardWindow(parent, false), m_windowStack(this), m_editing(false), m_previouslySelectedIndex(-1),
...
QSize QtProjectWizard::sizeHint() const { return QSize(900, 600); }
...
void QtProjectWizard::populateWindow(QWidget* widget)
{
...

   {
      ...
      QPushButton* addButton =
         new QtIconButton(ResourcePaths::getGuiDirectoryPath().concatenate(L"window/source_group_add.png"),
                          ResourcePaths::getGuiDirectoryPath().concatenate(L"window/source_group_add_hover.png"));

      m_removeButton =
         new QtIconButton(ResourcePaths::getGuiDirectoryPath().concatenate(L"window/source_group_delete.png"),
                          ResourcePaths::getGuiDirectoryPath().concatenate(L"window/source_group_delete_hover.png"));

      m_duplicateButton =
         new QtIconButton(ResourcePaths::getGuiDirectoryPath().concatenate(L"window/source_group_copy.png"),
...
void QtProjectWizard::handlePrevious() { QtWindow::handlePrevious(); }

QtProjectWizardWindow*
QtProjectWizard::createWindowWithContent(std::function<QtProjectWizardContent*(QtProjectWizardWindow*)> func)
{
...
void QtProjectWizard::generalButtonClicked()
{
   ...
   contentGroup->addContent(new QtProjectWizardContentProjectData(m_projectSettings, this, m_editing));
...
void QtProjectWizard::selectedSourceGroupChanged(int index)
{
   ...
   connect(content, &QtProjectWizardContentSourceGroupData::nameUpdated, this,
...
#if BUILD_JAVA_LANGUAGE_PACKAGE
   else if (std::shared_ptr<SourceGroupSettingsJavaEmpty> settings =
               std::dynamic_pointer_cast<SourceGroupSettingsJavaEmpty>(group)) {
      addSourceGroupContents(summary, settings, this);
   } else if (std::shared_ptr<SourceGroupSettingsJavaMaven> settings =
                 std::dynamic_pointer_cast<SourceGroupSettingsJavaMaven>(group)) {
      addSourceGroupContents(summary, settings, this);
   } else if (std::shared_ptr<SourceGroupSettingsJavaGradle> settings =
                 std::dynamic_pointer_cast<SourceGroupSettingsJavaGradle>(group)) {
      addSourceGroupContents(summary, settings, this);
   }
#endif // BUILD_JAVA_LANGUAGE_PACKAGE
#if BUILD_PYTHON_LANGUAGE_PACKAGE
   else if (std::shared_ptr<SourceGroupSettingsPythonEmpty> settings =
...
void QtProjectWizard::duplicateSelectedSourceGroup()
{
...

   {
      ...
      while (true) {
         ...
         for (const std::shared_ptr<SourceGroupSettings>& sourceGroupSettings : m_allSourceGroupSettings) {
...
void QtProjectWizard::cancelSourceGroup() { m_windowStack.clearWindows(); }
...
void QtProjectWizard::newSourceGroup()
{
   ...
   connect(dynamic_cast<QtProjectWizardContentSelect*>(window->content()), &QtProjectWizardContentSelect::selected,
           this, &QtProjectWizard::selectedProjectType);
