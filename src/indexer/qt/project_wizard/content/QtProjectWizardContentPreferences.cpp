...
QtProjectWizardContentPreferences::QtProjectWizardContentPreferences(QtProjectWizardWindow* window)
    : QtProjectWizardContent(window), m_oldColorSchemeIndex(-1), m_newColorSchemeIndex(-1),
      m_screenAutoScaling(nullptr), m_screenScaleFactor(nullptr)
{
...
void QtProjectWizardContentPreferences::populate(QGridLayout* layout, int& row)
{
   ...
   // font size
   m_fontSize = addComboBox(QStringLiteral("Font Size"), appSettings->getFontSizeMin(), appSettings->getFontSizeMax(),
   ...
   for (size_t i = 0; i < m_colorSchemePaths.size(); i++) {
      m_colorSchemes->insertItem(static_cast<int>(i),
                                 QString::fromStdWString(m_colorSchemePaths[i].withoutExtension().fileName()));
   }
   connect(m_colorSchemes, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this,
           &QtProjectWizardContentPreferences::colorSchemeChanged);

   // animations
   m_useAnimations =
      addCheckBox(QStringLiteral("Animations"), QStringLiteral("Enable animations"),
                  QStringLiteral("<p>Enable animations throughout the user interface.</p>"), layout, row);

   // built-in types
   m_showBuiltinTypes =
      addCheckBox(QStringLiteral("Built-in Types"), QStringLiteral("Show built-in types in graph when referenced"),
                  QStringLiteral("<p>Enable display of referenced built-in types in the graph view.</p>"), layout, row);

   // directory in code
   m_showDirectoryInCode =
      addCheckBox(QStringLiteral("Directory in File Title"), QStringLiteral("Show directory of file in code title"),
   ...
   if (utility::getOsType() == OS_LINUX) {
      ...
      m_screenAutoScaling = addComboBoxWithWidgets(
         ...
         QStringLiteral("<p>Define if automatic scaling to screen DPI resolution is active. "
                        ...
                        "effect.</p>"),
         {m_screenAutoScalingInfoLabel}, layout, row);
      ...
      connect(m_screenAutoScaling, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this,
      ...
      m_screenScaleFactor = addComboBoxWithWidgets(
         ...
         QStringLiteral("<p>Define a screen scale factor for the user interface of the application. "
                        ...
                        "<p>Changes to this setting require a restart of the application to take "
                        "effect.</p>"),
         {m_screenScaleFactorInfoLabel}, layout, row);
      ...
      connect(m_screenScaleFactor, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this,
   ...
   // scroll speed
   m_scrollSpeed =
      addLineEdit(QStringLiteral("Scroll Speed"),
   ...
   m_graphZooming = addCheckBox(QStringLiteral("Graph Zoom"), QStringLiteral("Zoom graph on mouse wheel"),
   ...
   // logging
   m_loggingEnabled =
      addCheckBox(QStringLiteral("Logging"), QStringLiteral("Enable console and file logging"),
                  QStringLiteral("<p>Show logs in the console and save this information in files.</p>"), layout, row);
   ...
   m_verboseIndexerLoggingEnabled =
      addCheckBox(QStringLiteral("Indexer Logging"), QStringLiteral("Enable verbose indexer logging"),
   ...
   addHelpButton(QStringLiteral("Log Directory Path"), QStringLiteral("<p>Log file will be saved to this path.</p>"),
                 layout, row);
   row++;
   ...
   // Sourcetrail port
   m_sourcetrailPort =
      addLineEdit(QStringLiteral("Sourcetrail Port"),
   ...
   m_pluginPort = addLineEdit(
      ...
      QStringLiteral("<p>Port number that Sourcetrail uses to sends outgoing messages to plugins.</p>"), layout, row);
   ...
   m_threads = addComboBoxWithWidgets(
      QStringLiteral("Indexer Threads"), 0, 24,
   ...
   connect(m_threads, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this,
           &QtProjectWizardContentPreferences::indexerThreadsChanges);

   // multi process indexing
   m_multiProcessIndexing =
      addCheckBox(QStringLiteral("Multi Process<br />C/C++ Indexing"),
...

   {
      ...
      addLabelAndWidget(("Java Path (" + javaArchitectureString + ")").c_str(), m_javaPath, layout, row);

      const std::string javaVersionString = javaArchitectureString + " Java 8";

      addHelpButton(QStringLiteral("Java Path"),
...

   {
      ...
      addHelpButton(QStringLiteral("JRE System Library"),
   ...
   }
   {
      ...
      addHelpButton(QStringLiteral("Maven Path"),
   ...
   m_pythonPostProcessing = addCheckBox(
      QStringLiteral("Post Processing"), QStringLiteral("Add ambiguous edges for unsolved references (recommended)"),
...
void QtProjectWizardContentPreferences::save()
{
   ...
   if (m_logPath && m_logPath->getText().toStdWString() != appSettings->getLogDirectoryPath().wstr()) {
      ...
      auto logger = LogManager::getInstance()->getLoggerByType("FileLogger");
      if (logger) {
         const auto fileLogger = dynamic_cast<FileLogger*>(logger);
         fileLogger->setLogDirectory(appSettings->getLogDirectoryPath().wstr());
...
bool QtProjectWizardContentPreferences::check() { return true; }
...
void QtProjectWizardContentPreferences::javaPathDetectionClicked()
{
   std::vector<FilePath> paths =
      m_javaPathDetector->getPathsForDetector(m_javaPathDetectorBox->currentText().toStdString());
   if (!paths.empty()) {
...
{
   std::vector<FilePath> paths =
      m_mavenPathDetector->getPathsForDetector(m_mavenPathDetectorBox->currentText().toStdString());
   if (!paths.empty()) {
...
void QtProjectWizardContentPreferences::addWidget(QWidget* widget, QGridLayout* layout, int row,
                                                  Qt::Alignment widgetAlignment)
{
   layout->addWidget(widget, row, QtProjectWizardWindow::BACK_COL, widgetAlignment);
}

void QtProjectWizardContentPreferences::addLabelAndWidget(const QString& label, QWidget* widget, QGridLayout* layout,
                                                          int row, Qt::Alignment widgetAlignment)
{
...
QCheckBox* QtProjectWizardContentPreferences::addCheckBox(const QString& label, const QString& text,
                                                          const QString& helpText, QGridLayout* layout, int& row)
{
...
QComboBox* QtProjectWizardContentPreferences::addComboBox(const QString& label, const QString& helpText,
...
QComboBox* QtProjectWizardContentPreferences::addComboBoxWithWidgets(const QString& label, const QString& helpText,
                                                                     std::vector<QWidget*> widgets, QGridLayout* layout,
                                                                     int& row)
{
   QComboBox* comboBox = new QComboBox(this);
...
QComboBox* QtProjectWizardContentPreferences::addComboBox(const QString& label, int min, int max,
                                                          const QString& helpText, QGridLayout* layout, int& row)
{
...
QComboBox* QtProjectWizardContentPreferences::addComboBoxWithWidgets(const QString& label, int min, int max,
...
QLineEdit* QtProjectWizardContentPreferences::addLineEdit(const QString& label, const QString& helpText,
                                                          QGridLayout* layout, int& row)
{
