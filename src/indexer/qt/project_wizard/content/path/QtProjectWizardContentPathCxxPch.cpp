...
QtProjectWizardContentPathCxxPch::QtProjectWizardContentPathCxxPch(
   std::shared_ptr<SourceGroupSettings> settings, std::shared_ptr<SourceGroupSettingsWithCxxPchOptions> settingsCxxPch,
    ...
    : QtProjectWizardContentPath(window), m_settings(settings), m_settingsCxxPch(settingsCxxPch)
{
   ...
   setHelpString("Specify the path to the input header file that should be used to generate a precompiled "
...
bool QtProjectWizardContentPathCxxPch::check()
{
   if (std::shared_ptr<SourceGroupSettingsCxxCdb> cdbSettings =
      ...
      auto const cdbPath = cdbSettings->getCompilationDatabasePathExpandedAndAbsolute();
      auto cdb = utility::loadCDB(cdbPath);
      if (!cdb) {
      ...
      if (utility::containsIncludePchFlags(cdb)) {
         if (m_settingsCxxPch->getPchInputFilePath().empty()) {
            QMessageBox msgBox(m_window);
            msgBox.setText("The provided compilation database file uses precompiled headers. If you want "
                           "to make use of "
            ...
            msgBox.addButton(QStringLiteral("Continue"), QMessageBox::ButtonRole::AcceptRole);
            msgBox.exec();
         ...
         if (!m_settingsCxxPch->getPchInputFilePath().empty()) {
            QMessageBox msgBox(m_window);
            msgBox.setText("The provided compilation database file does not use precompiled headers. The "
...
std::shared_ptr<SourceGroupSettings> QtProjectWizardContentPathCxxPch::getSourceGroupSettings() { return m_settings; }
