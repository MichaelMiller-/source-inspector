...
QtDialogView::QtDialogView(QtMainWindow* mainWindow, UseCase useCase, StorageAccess* storageAccess)
    : DialogView(useCase, storageAccess), m_mainWindow(mainWindow), m_parentWindow(nullptr), m_windowStack(this),
      m_resultReady(false)
{
}

QtDialogView::~QtDialogView() { m_resultReady = true; }

bool QtDialogView::dialogsHidden() const { return !m_dialogsVisible; }
...
void QtDialogView::showProgressDialog(const std::wstring& title, const std::wstring& message, size_t progress)
{
   m_onQtThread([=]() {
      ...
      if (!window) {
         ...
         sendStatusMessage =
            (window->getTitle() != title || window->getMessage() != message || window->getProgress() != progress);
      }
...
void QtDialogView::startIndexingDialog(Project* project, const std::vector<RefreshMode>& enabledModes,
                                       const RefreshMode initialMode, bool enabledShallowOption,
                                       ...
                                       std::function<void()> onCancelIndexing)
{
   ...
   m_onQtThread([=]() {
      ...
      QtIndexingStartDialog* window =
         createWindow<QtIndexingStartDialog>(enabledModes, initialMode, enabledShallowOption, initialShallowState);

      connect(window, &QtIndexingStartDialog::setShallowIndexing,
              [=](bool enabled) { m_shallowIndexingEnabled = enabled; });
      ...
      connect(window, &QtIndexingStartDialog::startIndexing, [=](RefreshMode refreshMode) {
         ...
         Task::dispatch(TabId::app(), std::make_shared<TaskLambda>([=]() { onStartIndexing(info); }));
...
void QtDialogView::updateIndexingDialog(size_t startedFileCount, size_t finishedFileCount, size_t totalFileCount,
   ...
   m_onQtThread([=]() {
      if (!sourcePaths.empty()) {
         ...
         for (const FilePath& path : sourcePaths) {
            stati.push_back(L"[" + std::to_wstring(startedFileCount) + L"/" + std::to_wstring(totalFileCount) +
      ...
      window->updateIndexingProgress(finishedFileCount, totalFileCount,
                                     sourcePaths.empty() ? FilePath() : sourcePaths.back());
...
void QtDialogView::updateCustomIndexingDialog(size_t startedFileCount, size_t finishedFileCount, size_t totalFileCount,
                                              const std::vector<FilePath>& sourcePaths)
{
...
DatabasePolicy QtDialogView::finishedIndexingDialog(size_t indexedFileCount, size_t totalIndexedFileCount,
                                                    size_t completedFileCount, size_t totalFileCount, float time,
                                                    ErrorCountInfo errorInfo, bool interrupted, bool shallow)
{
...
void QtDialogView::handleMessage(MessageWindowClosed* message) { m_resultReady = true; }

void QtDialogView::updateErrorCount(size_t errorCount, size_t fatalCount)
{
   if (QtIndexingProgressDialog* progressWindow =
          dynamic_cast<QtIndexingProgressDialog*>(m_windowStack.getTopWindow())) {
      progressWindow->updateErrorCount(errorCount, fatalCount);
   } else if (QtIndexingReportDialog* reportWindow =
                 dynamic_cast<QtIndexingReportDialog*>(m_windowStack.getTopWindow())) {
...
DialogType* QtDialogView::createWindow(ParamTypes... params)
{
   ...
   if (m_mainWindow) {
      connect(m_mainWindow, &QtMainWindow::hideIndexingDialog,
