...
QtErrorView::QtErrorView(ViewLayout* viewLayout) : ErrorView(viewLayout), m_controllerProxy(this, TabId::app())
{
   ...
   connect(m_table, &QTableView::clicked, [=, this](const QModelIndex& index) {
      if (index.isValid()) {
         ...
         const Id errorId = static_cast<Id>(m_model->item(index.row(), Column::ID)->text().toLongLong());
...

   {
      ...
      m_showNonIndexedFatals =
         createFilterCheckbox(QStringLiteral("Fatals in non-indexed files"), m_errorFilter.unindexedFatal, checkboxes);
      m_showNonIndexedErrors =
         createFilterCheckbox(QStringLiteral("Errors in non-indexed files"), m_errorFilter.unindexedError, checkboxes);
...
void QtErrorView::addErrors(const std::vector<ErrorInfo>& errors, const ErrorCountInfo& errorCount, bool scrollTo)
{
   m_onQtThread([=, this]() {
      ...
      m_errorLabel->setText(
         "<b>displaying " + QString::number(errorCount.total) + " error" + (errorCount.total != 1 ? "s" : "") +
         (errorCount.fatal > 0 ? " (" + QString::number(errorCount.fatal) + " fatal)" : QLatin1String("")) + "</b>");
   });
}
...
{
   m_onQtThread([=, this]() {
      auto items = m_model->findItems(QString::number(errorId), Qt::MatchExactly, Column::ID);
      if (items.size() == 1) {
...
ErrorFilter QtErrorView::getErrorFilter() const { return m_errorFilter; }
...
void QtErrorView::addErrorToTable(const ErrorInfo& error)
{
   ...
   m_model->setItem(rowNumber, Column::TYPE,
   ...
   m_model->setItem(rowNumber, Column::INDEXED,
                    new QStandardItem(error.indexed ? QStringLiteral("yes") : QStringLiteral("no")));

   m_model->setItem(rowNumber, Column::TRANSLATION_UNIT,
...
QCheckBox* QtErrorView::createFilterCheckbox(const QString& name, bool checked, QBoxLayout* layout) const
{
