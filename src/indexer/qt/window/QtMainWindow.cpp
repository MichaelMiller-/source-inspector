...
void QtViewToggle::clear() { m_view = nullptr; }
...
void QtMainWindow::showView(View* view) { getDockWidgetForView(view)->widget->setHidden(false); }

void QtMainWindow::hideView(View* view) { getDockWidgetForView(view)->widget->setHidden(true); }
...
void QtMainWindow::setWindowsTaskbarProgress(float progress) { m_windowsTaskbarButton.setProgress(progress); }

void QtMainWindow::hideWindowsTaskbarProgress() { m_windowsTaskbarButton.hideProgress(); }

void QtMainWindow::showEvent(QShowEvent* e) { m_windowsTaskbarButton.setWindow(this); }
...
void QtMainWindow::closeEvent(QCloseEvent* event) { MessageWindowClosed().dispatchImmediately(); }
...
void QtMainWindow::showErrorHelpMessage() { MessageErrorsHelpMessage(true).dispatch(); }
...
{
   QDesktopServices::openUrl(
      QUrl(QString::fromStdWString(L"file:///" + ApplicationSettings::getInstance()->getLogDirectoryPath().wstr()),
           QUrl::TolerantMode));
}

void QtMainWindow::openTab() { MessageTabOpen().dispatch(); }

void QtMainWindow::closeTab() { MessageTabClose().dispatch(); }

void QtMainWindow::nextTab() { MessageTabSelect(true).dispatch(); }

void QtMainWindow::previousTab() { MessageTabSelect(false).dispatch(); }
...
void QtMainWindow::hideStartScreen() { m_windowStack.clearWindows(); }
...
{
   QString fileName = QtFileDialog::getOpenFileName(this, tr("Open File"), FilePath(),
                                                    QStringLiteral("Sourcetrail Project Files (*.srctrlprj)"));
...
void QtMainWindow::find() { MessageFind().dispatch(); }

void QtMainWindow::findFulltext() { MessageFind(true).dispatch(); }

void QtMainWindow::findOnScreen() { emit showScreenSearch(); }
...
void QtMainWindow::codeReferenceNext() { MessageCodeReference(MessageCodeReference::REFERENCE_NEXT, false).dispatch(); }
...
void QtMainWindow::customTrail() { MessageCustomTrailShow().dispatch(); }

void QtMainWindow::overview() { MessageActivateOverview().dispatch(); }
...
void QtMainWindow::undo() { MessageHistoryUndo().dispatch(); }

void QtMainWindow::redo() { MessageHistoryRedo().dispatch(); }

void QtMainWindow::zoomIn() { MessageZoom(true).dispatch(); }

void QtMainWindow::zoomOut() { MessageZoom(false).dispatch(); }
...
void QtMainWindow::toggleShowDockWidgetTitleBars() { setShowDockWidgetTitleBars(!m_showDockWidgetTitleBars); }

void QtMainWindow::showBookmarkCreator() { MessageBookmarkCreate().dispatch(); }

void QtMainWindow::showBookmarkBrowser() { MessageBookmarkBrowse().dispatch(); }
...
void QtMainWindow::setupEditMenu()
{
   ...
   if (utility::getOsType() == OS_WINDOWS) {
      ...
      menu->addAction(tr("&Full Refresh"), this, &QtMainWindow::forceRefresh,
   ...
   menu->addAction(tr("Previous Reference"), this, &QtMainWindow::codeReferencePrevious,
                   QKeySequence(Qt::SHIFT + Qt::CTRL + Qt::Key_G));

   menu->addAction(tr("Next Local Reference"), this, &QtMainWindow::codeLocalReferenceNext,
                   QKeySequence(Qt::CTRL + Qt::Key_L));
   menu->addAction(tr("Previous Local Reference"), this, &QtMainWindow::codeLocalReferencePrevious,
   ...
   menu->addAction(tr("&Save Graph as Image..."), this, &QtMainWindow::saveAsImage,
...
void QtMainWindow::setupViewMenu()
{
   ...
   if (utility::getOsType() == OS_MAC) {
      ...
      menu->addAction(tr("Select Previous Tab"), this, &QtMainWindow::previousTab,
...
void QtMainWindow::setupBookmarksMenu()
{
   ...
   m_bookmarksMenu->addAction(tr("Bookmark Active Symbol..."), this, &QtMainWindow::showBookmarkCreator,
                              QKeySequence::Save);
   m_bookmarksMenu->addAction(tr("Bookmark Manager"), this, &QtMainWindow::showBookmarkBrowser,
