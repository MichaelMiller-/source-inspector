...
void GraphFocusHandler::focusTokenId(const std::list<QtGraphNode*>& nodes, const std::list<QtGraphEdge*>& edges,
                                     Id tokenId)
{
   m_focusNode = nullptr;
   m_focusEdge = nullptr;
...
QtGraphEdge* GraphFocusHandler::findNextEdge(QPointF pos, Direction direction, QtGraphEdge* previousEdge)
{
   ...
   for (QtGraphEdge* edge : m_client->getGraphEdges()) {
      ...
      bool isDir = (direction == Direction::UP && top) || (direction == Direction::DOWN && bottom) ||
...
QtGraphNode* GraphFocusHandler::findSibling(const QtGraphNode* node, Direction direction)
{
   ...
   auto rect = node->getBoundingRect();
   int minDist = 0;
   ...
   for (auto siblings : getSiblingsHierarchyRecursive(node)) {
      for (QtGraphNode* sibling : siblings) {
         ...
         auto rectS = sibling->getBoundingRect();
         ...
         const bool isDir = (direction == Direction::UP && (top || (!top && !bottom && !left && !right))) ||
         ...
         if (isDir) {
            ...
            int distYMult = (direction == Direction::LEFT || direction == Direction::RIGHT) ? 2 : 1;
            if (top)
