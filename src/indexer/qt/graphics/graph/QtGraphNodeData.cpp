...
QtGraphNodeData::QtGraphNodeData(GraphFocusHandler* focusHandler, const Node* data, const std::wstring& name,
                                 bool childVisible, bool hasQualifier, bool isInteractive)
    : QtGraphNode(focusHandler), m_data(data), m_childVisible(childVisible), m_hasQualifier(hasQualifier)
{
...
const Node* QtGraphNodeData::getData() const { return m_data; }
...
bool QtGraphNodeData::isDataNode() const { return true; }

Id QtGraphNodeData::getTokenId() const { return m_data->getId(); }
...
void QtGraphNodeData::onMiddleClick() { MessageTabOpenWith(m_data->getId()).dispatch(); }
