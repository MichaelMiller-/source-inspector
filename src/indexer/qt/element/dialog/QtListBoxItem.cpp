...
QtListBoxItem::QtListBoxItem(QListWidgetItem* item, QWidget* parent) : QWidget(parent), m_item(item), m_readOnly(false)
{
...
QString QtListBoxItem::getText() const { return m_data->text(); }

void QtListBoxItem::setText(const QString& text) { m_data->setText(text); }

bool QtListBoxItem::getReadOnly() const { return m_readOnly; }
...
void QtListBoxItem::setFocus() { m_data->setFocus(Qt::OtherFocusReason); }

void QtListBoxItem::selectItem() { getListBox()->selectItem(m_item); }

void QtListBoxItem::handleFocus() { selectItem(); }
