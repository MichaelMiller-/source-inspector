...
QtBookmark::QtBookmark(ControllerProxy<BookmarkController>* controllerProxy)
    : m_controllerProxy(controllerProxy), m_treeWidgetItem(nullptr), m_arrowImageName(L"arrow_line_down.png"),
      m_hovered(false), m_ignoreNextResize(false)
{
   ...
   m_deleteButton->setIcon(QPixmap(QString::fromStdWString(
      ResourcePaths::getGuiDirectoryPath().concatenate(L"bookmark_view/images/bookmark_delete_icon.png").wstr())));
...
Id QtBookmark::getBookmarkId() const { return m_bookmark->getId(); }

QTreeWidgetItem* QtBookmark::getTreeWidgetItem() const { return m_treeWidgetItem; }

void QtBookmark::setTreeWidgetItem(QTreeWidgetItem* treeWidgetItem) { m_treeWidgetItem = treeWidgetItem; }
...
void QtBookmark::showEvent(QShowEvent* event) { elideButtonText(); }
