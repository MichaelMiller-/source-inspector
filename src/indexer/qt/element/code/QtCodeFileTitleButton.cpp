...
QtCodeFileTitleButton::QtCodeFileTitleButton(QWidget* parent)
    : QtSelfRefreshIconButton(QLatin1String(""), FilePath(), "code/file/title", parent), m_isComplete(true),
      m_isIndexed(true)
{
...
const FilePath& QtCodeFileTitleButton::getFilePath() const { return m_filePath; }
...
bool QtCodeFileTitleButton::isComplete() const { return m_isComplete; }
...
bool QtCodeFileTitleButton::isIndexed() const { return m_isIndexed; }
...
void QtCodeFileTitleButton::updateHatching()
{
   if (!m_isIndexed) {
      ...
      setStyleSheet(
         QString::fromStdWString(L"#title_button { background-image: url(" + hatchingFilePath.wstr() + L"); }"));
   } else {
