...
QtCodeFileList::QtCodeFileList(QtCodeNavigator* navigator)
    : QFrame(), m_navigator(navigator), m_mirroredTitleBar(nullptr), m_mirroredSnippetScrollBar(nullptr)
{
...
QScrollArea* QtCodeFileList::getScrollArea() { return m_scrollArea; }
...
void QtCodeFileList::scrollTo(const FilePath& filePath, size_t lineNumber, Id locationId, Id scopeLocationId,
                              bool animated, CodeScrollParams::Target target, bool focusTarget)
{
   ...
   if (focusTarget) {
      m_navigator->setFocusedLocationId(snippet->getArea(), lineNumber,
                                        snippet->getArea()->getColumnNumberForLocationId(locationId), locationId, {},
                                        false, false);
   }
}
...
void QtCodeFileList::findScreenMatches(const std::wstring& query,
                                       std::vector<std::pair<QtCodeArea*, Id>>* screenMatches)
{
   for (QtCodeFile* file : m_files) {
...
void QtCodeFileList::updateSnippetTitleAndScrollBar()
{
   ...
   for (QtCodeFile* file : m_files) {
      ...
      if (!firstFile && visibleRect.top() > fileRect.top() && visibleRect.top() < fileRect.bottom() + 1 &&
          file->getVisibleSnippets().size()) {
         firstFile = file;
         fileTitleBarOffset =
            std::min(0, fileRect.bottom() + 1 - (visibleRect.top() + m_firstSnippetTitleBar->height()));
      }
...
void QtCodeFileList::updateFirstSnippetTitleBar(QtCodeFile* file, int fileTitleBarOffset)
{
   ...
   if (m_mirroredTitleBar != mirroredTitleBar) {
      ...
      if (m_mirroredTitleBar && file) {
         ...
         m_firstSnippetTitleBar->setGeometry(file->pos().x() + mirroredTitleBar->pos().x(), 0,
                                             mirroredTitleBar->width(), mirroredTitleBar->height());
...
void QtCodeFileList::updateLastSnippetScrollBar(QScrollBar* mirroredScrollBar)
{
   if (m_mirroredSnippetScrollBar != mirroredScrollBar) {
      ...
      if (mirroredScrollBar) {
         ...
         m_lastSnippetScrollBar->setGeometry(mirroredScrollBar->mapTo(m_scrollArea, mirroredScrollBar->pos()).x(),
                                             m_scrollArea->viewport()->size().height() - mirroredScrollBar->height(),
                                             mirroredScrollBar->width(), mirroredScrollBar->height());
