    ...
    : QWidget(parent), m_mode(MODE_NONE), m_oldMode(MODE_NONE), m_schedulerId(TabId::ignore())
{
...

   {
      ...
      auto buttonGroup = new QButtonGroup(navigation);
   ...
   //! \todo: Do not invoke virtual member functions from constructor
   updateFiles();
...
void QtCodeNavigator::addSnippetFile(const CodeFileParams& params) { m_list->addFile(params); }
...
void QtCodeNavigator::updateReferenceCount(size_t referenceCount, size_t referenceIndex, size_t localReferenceCount,
                                           size_t localReferenceIndex)
{
   ...
   if (localReferenceIndex != localReferenceCount) {
      m_localRefLabel->setText(QString::number(localReferenceIndex + 1) + "/" + QString::number(localReferenceCount) +
                               " local references");
   } else {
   ...
   m_localRefLabel->setMinimumWidth(
      m_localRefLabel->fontMetrics()
         .boundingRect(QString(QString::number(localReferenceCount).size() * 2, 'a') + "/ local references")
         .width() +
      30);
...
void QtCodeNavigator::clearCache() { m_single->clearCache(); }

void QtCodeNavigator::clearSnippetReferences() { m_list->clearSnippetTitleAndScrollBar(); }
...
Id QtCodeNavigator::getSchedulerId() const { return m_schedulerId; }

void QtCodeNavigator::setSchedulerId(Id schedulerId) { m_schedulerId = schedulerId; }

const std::set<Id>& QtCodeNavigator::getCurrentActiveTokenIds() const { return m_currentActiveTokenIds; }
...
const std::set<Id>& QtCodeNavigator::getCurrentActiveLocationIds() const { return m_currentActiveLocationIds; }
...
{
   m_currentActiveLocalLocationIds =
      std::set<Id>(currentActiveLocalLocationIds.begin(), currentActiveLocalLocationIds.end());
}

const std::set<Id>& QtCodeNavigator::getActiveTokenIds() const { return m_activeTokenIds; }
...
const std::set<Id>& QtCodeNavigator::getActiveLocalTokenIds() const { return m_activeLocalTokenIds; }
...
const std::set<Id>& QtCodeNavigator::getCoFocusedTokenIds() const { return m_coFocusedTokenIds; }
...
{
   auto it = m_errorInfos.find(errorId);
...
bool QtCodeNavigator::hasErrors() const { return not m_errorInfos.empty(); }
...
bool QtCodeNavigator::isInListMode() const { return m_mode == MODE_LIST; }

bool QtCodeNavigator::hasSingleFileCached(const FilePath& filePath) const { return m_single->hasFileCached(filePath); }
...
void QtCodeNavigator::focusInitialLocation(Id locationId)
{
   ...
   if (m_mode == MODE_LIST) {
      ...
      const auto locationId = m_single->getLocationIdOfFirstActiveLocationOfTokenId(0);
      if (locationId) {
...
void QtCodeNavigator::activateScreenMatch(size_t matchIndex)
{
   ...
   scrollTo(
      CodeScrollParams::toReference(p.first->getFilePath(), m_activeScreenMatchId, 0, CodeScrollParams::Target::CENTER),
      true, true);
}
...
bool QtCodeNavigator::hasScreenMatches() const { return !m_screenMatches.empty(); }
...
void QtCodeNavigator::scrollTo(const CodeScrollParams& params, bool animated, bool focusTarget)
{
   ...
   case CodeScrollParams::Type::TO_REFERENCE:
      func = [=, this]() {
         m_current->scrollTo(params.filePath, 0, params.locationId, params.scopeLocationId, animated, params.target,
                             focusTarget);
      };
      break;
   case CodeScrollParams::Type::TO_FILE:
      func = [=, this]() { m_current->scrollTo(params.filePath, 0, 0, 0, animated, params.target, focusTarget); };
      break;
   case CodeScrollParams::Type::TO_LINE:
      func = [=, this]() { m_current->scrollTo(params.filePath, params.line, 0, 0, animated, params.target, focusTarget); };
      break;
...
void QtCodeNavigator::scrollToFocus()
{
   ...
   if (focus.file) {
      ...
      scrollTo(CodeScrollParams::toLine(focus.area->getFilePath(), focus.lineNumber, CodeScrollParams::Target::VISIBLE),
               true, false);
   } else if (focus.locationId) {
      scrollTo(CodeScrollParams::toReference(focus.area->getFilePath(), focus.locationId, 0,
                                             CodeScrollParams::Target::VISIBLE),
               true, false);
   }
}

void QtCodeNavigator::scrolled(int value) { MessageScrollCode(value, m_mode == MODE_LIST).dispatch(); }

void QtCodeNavigator::showEvent(QShowEvent* event) { scrollTo(m_scrollParams, false, true); }
...
void QtCodeNavigator::focusInEvent(QFocusEvent* event) { emit focusIn(); }
...
void QtCodeNavigator::nextReference() { MessageCodeReference(MessageCodeReference::REFERENCE_NEXT, false).dispatch(); }
