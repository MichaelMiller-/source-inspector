...
void QtCodeFileSingle::clearFile() { setFileData(FileData()); }
...
bool QtCodeFileSingle::addFile(const CodeFileParams& params, bool useSingleFileCache)
{
   ...
   auto locationFile = params.fileParams->locationFile;
   ...
   file.area =
      new QtCodeArea(1, params.fileParams->code, locationFile, m_navigator, !params.fileParams->isOverview, this);
...
QAbstractScrollArea* QtCodeFileSingle::getScrollArea() { return m_area; }
...
void QtCodeFileSingle::scrollTo(const FilePath& filePath, size_t lineNumber, Id locationId, Id scopeLocationId,
...
void QtCodeFileSingle::onWindowFocus() { m_titleBar->getTitleButton()->updateTexts(); }

void QtCodeFileSingle::findScreenMatches(const std::wstring& query,
...
bool QtCodeFileSingle::hasFileCached(const FilePath& filePath) const { return getFileData(filePath).area != nullptr; }
...
void QtCodeFileSingle::clickedSnippetButton()
{
   ...
   MessageChangeFileView(m_currentFilePath, MessageChangeFileView::FILE_SNIPPETS, MessageChangeFileView::VIEW_LIST,
                         CodeScrollParams::toFile(m_currentFilePath, CodeScrollParams::Target::TOP), true)
      .dispatch();
}
