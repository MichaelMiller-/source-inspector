...
QSize QtLineNumberArea::sizeHint() const { return QSize(m_codeArea->lineNumberAreaWidth(), 0); }

void QtLineNumberArea::paintEvent(QPaintEvent* event) { m_codeArea->lineNumberAreaPaintEvent(event); }

QtCodeArea::QtCodeArea(size_t startLineNumber, const std::string& code,
                       std::shared_ptr<SourceLocationFile> locationFile, QtCodeNavigator* navigator,
                       bool showLineNumbers, QWidget* parent)
    : QtCodeField(startLineNumber, code, locationFile, parent), m_navigator(navigator), m_digits(0),
      m_isSelecting(false), m_isPanning(false), m_isDragging(false), m_setIDECursorPositionAction(nullptr),
      m_eventPosition(0, 0), m_isActiveFile(false), m_showLineNumbers(showLineNumbers)
{
...
void QtCodeArea::lineNumberAreaPaintEvent(QPaintEvent* event)
{
   ...
   utility::setWidgetBackgroundColor(m_lineNumberArea,
...
int QtCodeArea::lineNumberDigits() const { return static_cast<int>(utility::digits(getEndLineNumber())); }
...
int QtCodeArea::lineHeight() const { return static_cast<int>(blockBoundingRect(firstVisibleBlock()).height()); }
...
void QtCodeArea::updateContent() { annotateText(); }

void QtCodeArea::setIsActiveFile(bool isActiveFile) { m_isActiveFile = isActiveFile; }
...
QRectF QtCodeArea::getLineRectForLineNumber(size_t lineNumber) const
{
   ...
   QTextBlock block = document()->findBlockByLineNumber(static_cast<int>(lineNumber - getStartLineNumber()));
...
void QtCodeArea::ensureLocationIdVisible(Id locationId, int parentWidth, bool animated)
{
   ...
   const int newValue =
      static_cast<int>((scrollBar->maximum() - scrollBar->minimum()) * percentTarget + scrollBar->minimum());
...
bool QtCodeArea::moveFocus(CodeFocusHandler::Direction direction, size_t lineNumber, Id locationId)
{
   ...
   case CodeFocusHandler::Direction::UP:
      return moveFocusToLine(static_cast<int>(lineNumber) - 1, static_cast<int>(m_navigator->getTargetColumn()), true);
   case CodeFocusHandler::Direction::DOWN:
      return moveFocusToLine(static_cast<int>(lineNumber) + 1, static_cast<int>(m_navigator->getTargetColumn()), false);
...
void QtCodeArea::defocusTokenIds(const std::vector<Id>& tokenIds) { MessageFocusOut(tokenIds).dispatch(); }
...
void QtCodeArea::setCopyAvailable(bool yes) { m_copyAction->setEnabled(yes); }
...
void QtCodeArea::focusAnnotation(const Annotation* annotation, bool updateTargetColumn, bool fromMouse)
{
   ...
   m_navigator->setFocusedLocationId(this, annotation->startLine, annotation->startCol + 1, annotation->locationId,
                                     utility::toVector(annotation->tokenIds), updateTargetColumn, fromMouse);
}
...
void QtCodeArea::annotateText()
{
   ...
   bool needsUpdate = QtCodeField::annotateText(activeSymbolIds, activeLocationIds, coFocusedSymbolIds,
