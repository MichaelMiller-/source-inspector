...
ProjectSettings::ProjectSettings(const FilePath& projectFilePath) { setFilePath(projectFilePath); }
...
bool ProjectSettings::reload() { return Settings::load(getFilePath()); }

FilePath ProjectSettings::getProjectFilePath() const { return getFilePath(); }
...
FilePath ProjectSettings::getDBFilePath() const { return getFilePath().replaceExtension(INDEX_DB_FILE_EXTENSION); }
...
std::wstring ProjectSettings::getProjectName() const { return getFilePath().withoutExtension().fileName(); }

FilePath ProjectSettings::getProjectDirectoryPath() const { return getFilePath().getParentDirectory(); }

std::string ProjectSettings::getDescription() const { return getValue<std::string>("description", ""); }
...
SettingsMigrator ProjectSettings::getMigrations() const
{
   ...
   migrator.addMigration(
      1, std::make_shared<SettingsMigrationLambda>([](const SettingsMigration* migration, Settings* settings) {
         ...
         const std::string standard =
            migration->getValueFromSettings<std::string>(settings, "language_settings/standard", "");
...

   {
      ...
      migrator.addMigration(
         2, std::make_shared<SettingsMigrationMoveKey>("source/extensions/source_extensions",
                                                       sourceGroupKey + "/source_extensions/source_extension"));
      migrator.addMigration(
         2, std::make_shared<SettingsMigrationMoveKey>("source/header_search_paths/header_search_path",
                                                       sourceGroupKey + "/header_search_paths/header_search_path"));
      migrator.addMigration(
         2, std::make_shared<SettingsMigrationMoveKey>("source/use_source_paths_for_header_search",
      ...
      migrator.addMigration(
         2, std::make_shared<SettingsMigrationMoveKey>("source/compiler_flags/compiler_flag",
                                                       sourceGroupKey + "/compiler_flags/compiler_flag"));
      migrator.addMigration(
         2, std::make_shared<SettingsMigrationMoveKey>("source/build_file_path/compilation_db_path",
                                                       sourceGroupKey + "/build_file_path/compilation_db_path"));
      migrator.addMigration(2, std::make_shared<SettingsMigrationMoveKey>("source/class_paths/class_path",
                                                                          sourceGroupKey + "/class_paths/class_path"));
      migrator.addMigration(2, std::make_shared<SettingsMigrationMoveKey>("source/maven/project_file_path",
                                                                          sourceGroupKey + "/maven/project_file_path"));
      migrator.addMigration(
         2, std::make_shared<SettingsMigrationMoveKey>("source/maven/dependencies_directory",
      ...
      migrator.addMigration(
         3, std::make_shared<SettingsMigrationLambda>([=](const SettingsMigration* migration, Settings* settings) {
            ...
            if (language == "Java") {
               ...
               const std::string gradleProjectFilePath = migration->getValueFromSettings<std::string>(
   ...
   for (std::shared_ptr<SourceGroupSettings> sourceGroupSettings : getAllSourceGroupSettings()) {
      ...
      migrator.addMigration(5, std::make_shared<SettingsMigrationMoveKey>(key + "/exclude_paths/exclude_path",
                                                                          key + "/exclude_filters/exclude_filter"));
   }
   ...
   for (std::shared_ptr<SourceGroupSettings> sourceGroupSettings : getAllSourceGroupSettings()) {
      ...
      migrator.addMigration(
         7, std::make_shared<SettingsMigrationMoveKey>(key + "/standard", key + "/" + languageName + "_standard"));
   }

   for (std::shared_ptr<SourceGroupSettings> sourceGroupSettings : getAllSourceGroupSettings()) {
      const std::string key = SourceGroupSettings::s_keyPrefix + sourceGroupSettings->getId();
      migrator.addMigration(8, std::make_shared<SettingsMigrationMoveKey>(key + "/python_environment_directory_path",
                                                                          key + "/python_environment_path"));
   }
