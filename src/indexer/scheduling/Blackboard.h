...
template <typename T> struct BlackboardItem : public BlackboardItemBase
{
...
public:
   ...
   template <typename T> bool update(const std::string& key, std::function<T(const T&)> updater);
...
template <typename T> bool Blackboard::get(const std::string& key, T& value)
{
   std::lock_guard<std::mutex> lock(m_itemMutex);
...
template <typename T> bool Blackboard::update(const std::string& key, std::function<T(const T&)> updater)
{
   std::lock_guard<std::mutex> lock(m_itemMutex);
