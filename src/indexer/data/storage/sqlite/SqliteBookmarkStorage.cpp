...
size_t SqliteBookmarkStorage::getStaticVersion() const { return s_storageVersion; }
...
void SqliteBookmarkStorage::migrateIfNecessary()
{
   ...
   migrator.addMigration(
      2, std::make_shared<SqliteStorageMigrationLambda>([](const SqliteStorageMigration* migration,
         ...
         if (Application::getInstance()) {
            std::shared_ptr<const Project> currentProject = Application::getInstance()->getCurrentProject();
            {
               LanguageType currentLanguage =
                  ProjectSettings::getLanguageOfProject(currentProject->getProjectSettingsFilePath());
         ...
         migration->executeStatementInStorage(storage, "UPDATE bookmarked_node SET serialized_node_name = '" +
                                                          separator + "\tm' || serialized_node_name");
         migration->executeStatementInStorage(storage, "UPDATE bookmarked_edge SET serialized_source_node_name = '" +
                                                          separator + "\tm' || serialized_source_node_name");
         migration->executeStatementInStorage(storage, "UPDATE bookmarked_edge SET serialized_target_node_name = '" +
                                                          separator + "\tm' || serialized_target_node_name");
      }));
...
StorageBookmarkedEdge SqliteBookmarkStorage::addBookmarkedEdge(const StorageBookmarkedEdgeData data)
{
   ...
   std::string statement = "INSERT INTO bookmarked_edge(id, serialized_source_node_name, serialized_target_node_name, "
...
std::vector<StorageBookmark> SqliteBookmarkStorage::getAllBookmarks() const { return doGetAll<StorageBookmark>(""); }
...
void SqliteBookmarkStorage::updateBookmark(const Id bookmarkId, const std::wstring& name, const std::wstring& comment,
                                           const Id categoryId)
{
   executeStatement("UPDATE bookmark SET name = '" + utility::encodeToUtf8(name) +
...
{
   try {
      m_database.execDML("CREATE TABLE IF NOT EXISTS bookmark_category("
                         "id INTEGER NOT NULL, "
                         "name TEXT, "
                         "PRIMARY KEY(id)"
                         ");");

      m_database.execDML("CREATE TABLE IF NOT EXISTS bookmark("
                         "id INTEGER NOT NULL, "
                         "name TEXT, "
                         "comment TEXT, "
                         "timestamp TEXT, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS bookmarked_element("
                         "id INTEGER NOT NULL, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS bookmarked_node("
                         "id INTEGER NOT NULL, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS bookmarked_edge("
                         "id INTEGER NOT NULL, "
...
template <>
std::vector<StorageBookmarkCategory>
SqliteBookmarkStorage::doGetAll<StorageBookmarkCategory>(const std::string& query) const
{
...
{
   CppSQLite3Query q = executeQuery("SELECT id, name, comment, timestamp, category_id FROM bookmark " + query + ";");
   ...
   while (!q.eof()) {
      ...
      if (id != 0 && name != "" && timestamp != "") {
         bookmarks.push_back(StorageBookmark(id, utility::decodeFromUtf8(name), utility::decodeFromUtf8(comment),
...
template <>
std::vector<StorageBookmarkedNode>
SqliteBookmarkStorage::doGetAll<StorageBookmarkedNode>(const std::string& query) const
{
   CppSQLite3Query q =
      executeQuery("SELECT "
...
template <>
std::vector<StorageBookmarkedEdge>
SqliteBookmarkStorage::doGetAll<StorageBookmarkedEdge>(const std::string& query) const
{
   CppSQLite3Query q =
      executeQuery("SELECT "
   ...
   while (!q.eof()) {
      ...
      if (id != 0 && bookmarkId != 0 && serializedSourceNodeName != "" && serializedTargetNodeName != "" &&
          edgeType != -1 && sourceNodeActive != -1) {
         bookmarkedEdges.push_back(
            StorageBookmarkedEdge(id, bookmarkId, utility::decodeFromUtf8(serializedSourceNodeName),
                                  utility::decodeFromUtf8(serializedTargetNodeName), edgeType, sourceNodeActive));
      }
