...
size_t SqliteIndexStorage::getStorageVersion() { return s_storageVersion; }

SqliteIndexStorage::SqliteIndexStorage(const FilePath& dbFilePath) : SqliteStorage(dbFilePath.getCanonical()) {}

size_t SqliteIndexStorage::getStaticVersion() const { return s_storageVersion; }
...
std::string SqliteIndexStorage::getProjectSettingsText() const { return getMetaValue("project_settings"); }

void SqliteIndexStorage::setProjectSettingsText(std::string text) { insertOrUpdateMetaValue("project_settings", text); }
...
bool SqliteIndexStorage::addSymbol(const StorageSymbol& data) { return addSymbols({data}); }
...
std::vector<Id> SqliteIndexStorage::addEdges(const std::vector<StorageEdge>& edges)
{
   if (m_tempEdgeIndex.empty()) {
      forEach<StorageEdge>([this](StorageEdge&& edge) {
         m_tempEdgeIndex.emplace(StorageEdgeData(edge.type, edge.sourceNodeId, edge.targetNodeId),
...
std::vector<Id> SqliteIndexStorage::addLocalSymbols(const std::set<StorageLocalSymbol>& symbols)
{
   if (m_tempLocalSymbolIndex.empty()) {
      forEach<StorageLocalSymbol>([this](StorageLocalSymbol&& localSymbol) {
         ...
         if (name.second.size()) {
            m_tempLocalSymbolIndex[name.first].emplace(name.second, static_cast<uint32_t>(localSymbol.id));
         }
      });
   }
...
std::vector<Id> SqliteIndexStorage::addSourceLocations(const std::vector<StorageSourceLocation>& locations)
{
   if (m_tempSourceLocationIndices.empty()) {
      forEach<StorageSourceLocation>([this](StorageSourceLocation&& loc) {
         ...
         index.emplace(
            TempSourceLocation(static_cast<uint32_t>(loc.startLine), static_cast<uint16_t>(loc.endLine - loc.startLine),
                               static_cast<uint16_t>(loc.startCol), static_cast<uint16_t>(loc.endCol), loc.type),
   ...
   for (size_t i = 0; i < locations.size(); i++) {
      ...
      const TempSourceLocation tempLoc(
         static_cast<uint32_t>(data.startLine), static_cast<uint16_t>(data.endLine - data.startLine),
         static_cast<uint16_t>(data.startCol), static_cast<uint16_t>(data.endCol), data.type);
...
bool SqliteIndexStorage::addOccurrence(const StorageOccurrence& data) { return addOccurrences({data}); }
                                                           ...
                                                           std::function<void(int)> updateStatusCallback)
{
   ...
   executeStatement("CREATE TABLE IF NOT EXISTS element_id_to_clear("
                    "id INTEGER NOT NULL, "
                    "PRIMARY KEY(id));");
   ...
   executeStatement("INSERT INTO element_id_to_clear "
   ...
   executeStatement("DELETE FROM element WHERE element.id IN "
   ...
   executeStatement("DELETE FROM element WHERE element.id IN (SELECT id FROM edge WHERE source_node_id IN "
   ...
   // can disregard them)
   executeStatement("DELETE FROM element_id_to_clear WHERE id NOT IN ("
   ...
   executeStatement("DELETE FROM element_id_to_clear WHERE id IN ("
                    "	SELECT id FROM file"
                    ")");
   ...
   executeStatement("DELETE FROM source_location WHERE file_node_id IN (" +
                    utility::join(utility::toStrings(fileIds), ',') + ");");
   ...
   executeStatement("DELETE FROM element WHERE EXISTS ("
...
void SqliteIndexStorage::removeAllErrors() { executeStatement("DELETE FROM error;"); }
...
{
   return doGetAll<StorageEdge>("WHERE source_node_id == " + std::to_string(id) +
...
StorageNode SqliteIndexStorage::getNodeBySerializedName(const std::wstring& serializedName) const
{
   CppSQLite3Statement stmt =
      m_database.compileStatement("SELECT id, type, serialized_name FROM node WHERE serialized_name == ? LIMIT 1;");
...
{
   CppSQLite3Query q = executeQuery("SELECT content FROM filecontent WHERE id = '" + std::to_string(fileId) + "';");
   if (!q.eof()) {
...
{
   try {
      CppSQLite3Query q = executeQuery("SELECT filecontent.content "
                                       "FROM filecontent "
...
{
   bool fileHasErrors =
      doGetFirst<StorageSourceLocation>("WHERE file_node_id == " + std::to_string(fileId) +
                                        " AND type == " + std::to_string(locationTypeToInt(LOCATION_ERROR)))
         .id;
   if (fileHasErrors != complete) {
      executeStatement("UPDATE file SET complete = " + std::to_string(complete) +
                                                                                  ...
                                                                                  const std::string& query) const
{
   ...
   for (const StorageSourceLocation& location : sourceLocations) {
      ...
      ret->addSourceLocation(intToLocationType(location.type), location.id,
...
SqliteIndexStorage::getSourceLocationsForLinesInFile(const FilePath& filePath, size_t startLine, size_t endLine) const
{
   return getSourceLocationsForFile(filePath, "AND start_line <= " + std::to_string(endLine) +
...
SqliteIndexStorage::getSourceLocationsForElementIds(const std::vector<Id>& elementIds) const
{
   std::vector<Id> sourceLocationIds;
   std::map<Id, std::vector<Id>> sourceLocationIdToElementIds;
   ...
   CppSQLite3Query q = executeQuery("SELECT source_location.id, file.path, source_location.start_line, "
   ...
   while (!q.eof()) {
      ...
      if (id != 0 && filePath.size() && startLineNumber != -1 && startColNumber != -1 && endLineNumber != -1 &&
         ...
         ret->addSourceLocation(intToLocationType(type), id, sourceLocationIdToElementIds[id],
                                FilePath(utility::decodeFromUtf8(filePath)), startLineNumber, startColNumber,
...
std::vector<StorageOccurrence>
SqliteIndexStorage::getOccurrencesForLocationIds(const std::vector<Id>& locationIds) const
{
   return doGetAll<StorageOccurrence>("WHERE source_location_id IN (" +
                                      utility::join(utility::toStrings(locationIds), ',') + ")");
}
...
std::vector<StorageComponentAccess>
SqliteIndexStorage::getComponentAccessesByNodeIds(const std::vector<Id>& nodeIds) const
{
   return doGetAll<StorageComponentAccess>("WHERE node_id IN (" + utility::join(utility::toStrings(nodeIds), ',') +
                                           ")");
}

std::vector<StorageElementComponent>
SqliteIndexStorage::getElementComponentsByElementIds(const std::vector<Id>& elementIds) const
{
   return doGetAll<StorageElementComponent>("WHERE element_id IN (" +
                                            utility::join(utility::toStrings(elementIds), ',') + ")");
}
...
std::vector<ErrorInfo> SqliteIndexStorage::getAllErrorInfos() const
{
   ...
   CppSQLite3Query q =
      executeQuery("SELECT error.id, error.message, error.fatal, error.indexed, error.translation_unit, "
   ...
   while (!q.eof()) {
      ...
      if (id != 0) {
         ...
         errorInfos.push_back(ErrorInfo(errorId, utility::decodeFromUtf8(message), utility::decodeFromUtf8(filePath),
                                        lineNumber, columnNumber, utility::decodeFromUtf8(translationUnit), fatal,
                                        indexed));
      }
...
int SqliteIndexStorage::getNodeCount() const { return executeStatementScalar("SELECT COUNT(*) FROM node;", 0); }

int SqliteIndexStorage::getEdgeCount() const { return executeStatementScalar("SELECT COUNT(*) FROM edge;", 0); }
...
std::vector<std::pair<int, SqliteDatabaseIndex>> SqliteIndexStorage::getIndices() const
{
   ...
   indices.push_back(
      std::make_pair(STORAGE_MODE_CLEAR, SqliteDatabaseIndex("edge_source_node_id_index", "edge(source_node_id)")));
   indices.push_back(
      std::make_pair(STORAGE_MODE_CLEAR, SqliteDatabaseIndex("edge_target_node_id_index", "edge(target_node_id)")));
   indices.push_back(std::make_pair(STORAGE_MODE_READ | STORAGE_MODE_CLEAR,
                                    SqliteDatabaseIndex("node_serialized_name_index", "node(serialized_name)")));
   indices.push_back(
      std::make_pair(STORAGE_MODE_READ | STORAGE_MODE_CLEAR,
                     SqliteDatabaseIndex("source_location_file_node_id_index", "source_location(file_node_id)")));
   indices.push_back(
      std::make_pair(STORAGE_MODE_WRITE, SqliteDatabaseIndex("error_all_data_index", "error(message, fatal)")));
   indices.push_back(std::make_pair(STORAGE_MODE_WRITE, SqliteDatabaseIndex("file_path_index", "file(path)")));
   indices.push_back(std::make_pair(STORAGE_MODE_READ | STORAGE_MODE_CLEAR,
   ...
   indices.push_back(
      std::make_pair(STORAGE_MODE_CLEAR, SqliteDatabaseIndex("edge_source_foreign_key_index", "edge(source_node_id)")));
   indices.push_back(
      std::make_pair(STORAGE_MODE_CLEAR, SqliteDatabaseIndex("edge_target_foreign_key_index", "edge(target_node_id)")));
   indices.push_back(std::make_pair(
      STORAGE_MODE_CLEAR, SqliteDatabaseIndex("source_location_foreign_key_index", "source_location(file_node_id)")));
   indices.push_back(std::make_pair(
      STORAGE_MODE_CLEAR, SqliteDatabaseIndex("occurrence_element_foreign_key_index", "occurrence(element_id)")));
   indices.push_back(
      std::make_pair(STORAGE_MODE_CLEAR, SqliteDatabaseIndex("occurrence_source_location_foreign_key_index",
...
{
   try {
      m_database.execDML("CREATE TABLE IF NOT EXISTS element("
                         "id INTEGER, "
                         "PRIMARY KEY(id));");

      m_database.execDML("CREATE TABLE IF NOT EXISTS element_component("
                         "	id INTEGER, "
                         "	element_id INTEGER, "
                         "	type INTEGER, "
                         "	data TEXT, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS edge("
                         "id INTEGER NOT NULL, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS node("
                         "id INTEGER NOT NULL, "
                         "type INTEGER NOT NULL, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS symbol("
                         "id INTEGER NOT NULL, "
                         "definition_kind INTEGER NOT NULL, "
                         "PRIMARY KEY(id), "
                         "FOREIGN KEY(id) REFERENCES node(id) ON DELETE CASCADE);");

      m_database.execDML("CREATE TABLE IF NOT EXISTS file("
                         "id INTEGER NOT NULL, "
                         "path TEXT, "
                         "language TEXT, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS filecontent("
                         "id INTEGER, "
                         "content TEXT, "
                         "PRIMARY KEY(id), "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS local_symbol("
                         "id INTEGER NOT NULL, "
                         "name TEXT, "
                         "PRIMARY KEY(id), "
                         "FOREIGN KEY(id) REFERENCES element(id) ON DELETE CASCADE);");

      m_database.execDML("CREATE TABLE IF NOT EXISTS source_location("
                         "id INTEGER NOT NULL, "
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS occurrence("
      ...
      m_database.execDML("CREATE TABLE IF NOT EXISTS component_access("
                         "node_id INTEGER NOT NULL, "
                         "type INTEGER NOT NULL, "
                         "PRIMARY KEY(node_id), "
                         "FOREIGN KEY(node_id) REFERENCES node(id) ON DELETE CASCADE);");

      m_database.execDML("CREATE TABLE IF NOT EXISTS error("
                         "id INTEGER NOT NULL, "
                         "message TEXT, "
...
void SqliteIndexStorage::setupPrecompiledStatements()
{
   try {
      m_insertNodeBatchStatement.compile(
         "INSERT INTO node(id, type, serialized_name) VALUES", 3,
      ...
      m_insertEdgeBatchStatement.compile(
         "INSERT INTO edge(id, type, source_node_id, target_node_id) VALUES", 4,
      ...
      m_insertSymbolBatchStatement.compile(
         "INSERT OR IGNORE INTO symbol(id, definition_kind) VALUES", 2,
      ...
      m_insertLocalSymbolBatchStatement.compile(
         "INSERT INTO local_symbol(id, name) VALUES", 2,
      ...
      m_insertOccurrenceBatchStatement.compile(
         "INSERT OR IGNORE INTO occurrence(element_id, source_location_id) VALUES", 2,
      ...
      m_insertComponentAccessBatchStatement.compile(
         "INSERT OR IGNORE INTO component_access(node_id, type) VALUES", 2,
      ...
      m_insertFileStmt =
         m_database.compileStatement("INSERT INTO file(id, path, language, modification_time, indexed, complete, "
                                     "line_count) VALUES(?, ?, ?, ?, ?, ?, ?);");
      m_insertFileContentStmt = m_database.compileStatement("INSERT INTO filecontent(id, content) VALUES(?, ?);");
      m_checkErrorExistsStmt = m_database.compileStatement("SELECT id FROM error WHERE "
                                                           "message = ? AND "
                                                           "fatal == ? "
                                                           "LIMIT 1;");
      m_insertErrorStmt =
         m_database.compileStatement("INSERT INTO error(id, message, fatal, indexed, translation_unit) "
                                     "VALUES(?, ?, ?, ?, ?);");
   } catch (CppSQLite3Exception& e) {
      LOG_ERROR(std::to_string(e.errorCode()) + ": " + e.errorMessage());
...
template <>
void SqliteIndexStorage::forEach<StorageSymbol>(const std::string& query,
                                                std::function<void(StorageSymbol&&)> func) const
{
...
{
   CppSQLite3Query q =
      executeQuery("SELECT id, path, language, modification_time, indexed, complete FROM file " + query + ";");
   ...
   while (!q.eof()) {
      ...
      if (id != 0) {
         func(StorageFile(id, utility::decodeFromUtf8(filePath), utility::decodeFromUtf8(languageIdentifier),
                          modificationTime, indexed, complete));
      }
      q.nextRow();
   }
}
...
{
   CppSQLite3Query q =
      executeQuery("SELECT id, file_node_id, start_line, start_column, end_line, end_column, type FROM "
                   "source_location " +
                   query + ";");
   ...
   while (!q.eof()) {
      ...
      if (id != 0 && fileNodeId != 0 && startLineNumber != -1 && startColNumber != -1 && endLineNumber != -1 &&
          endColNumber != -1 && type != -1) {
         func(
            StorageSourceLocation(id, fileNodeId, startLineNumber, startColNumber, endLineNumber, endColNumber, type));
      }
...
{
   CppSQLite3Query q = executeQuery("SELECT element_id, type, data FROM element_component " + query + ";");
...
void SqliteIndexStorage::forEach<StorageError>(const std::string& query, std::function<void(StorageError&&)> func) const
{
   ...
   while (!q.eof()) {
      ...
      if (id != 0) {
         func(StorageError(id, utility::decodeFromUtf8(message), utility::decodeFromUtf8(translationUnit), fatal,
                           indexed));
      }
