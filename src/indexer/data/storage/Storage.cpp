...

   {
      ...
      for (const StorageFile& file : injected->getStorageFiles()) {
         ...
         if (it != injectedIdToOwnElementId.end()) {
            addFile(StorageFile(it->second, file.filePath, file.languageIdentifier, file.modificationTime, file.indexed,
                                file.complete));
         }
      }
   }
...

   {
      ...
      for (const StorageSourceLocation& location : oldLocations) {
         ...
         if (it != injectedIdToOwnElementId.end()) {
            ...
            locations.emplace_back(location.id, ownFileNodeId, location.startLine, location.startCol, location.endLine,
                                   location.endCol, location.type);
         }
      }
