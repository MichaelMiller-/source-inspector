...
void PersistentStorage::addSymbol(const StorageSymbol& data) { m_sqliteIndexStorage.addSymbol(data); }
...
Id PersistentStorage::addEdge(const StorageEdgeData& data) { return m_sqliteIndexStorage.addEdge(data); }
...
void PersistentStorage::addOccurrence(const StorageOccurrence& data) { m_sqliteIndexStorage.addOccurrence(data); }
...
void PersistentStorage::setMode(const SqliteIndexStorage::StorageModeType mode) { m_sqliteIndexStorage.setMode(mode); }

FilePath PersistentStorage::getIndexDbFilePath() const { return m_sqliteIndexStorage.getDbFilePath(); }

FilePath PersistentStorage::getBookmarkDbFilePath() const { return m_sqliteBookmarkStorage.getDbFilePath(); }

bool PersistentStorage::isEmpty() const { return m_sqliteIndexStorage.isEmpty(); }

bool PersistentStorage::isIncompatible() const { return m_sqliteIndexStorage.isIncompatible(); }

std::string PersistentStorage::getProjectSettingsText() const { return m_sqliteIndexStorage.getProjectSettingsText(); }

void PersistentStorage::setProjectSettingsText(std::string text) { m_sqliteIndexStorage.setProjectSettingsText(text); }
...
Id PersistentStorage::getNodeIdForFileNode(const FilePath& filePath) const { return getFileNodeId(filePath); }
...
std::map<Id, std::pair<Id, NameHierarchy>>
PersistentStorage::getNodeIdToParentFileMap(const std::vector<Id>& nodeIds) const
{
      ...
      if (location->isStartLocation() && location->isScopeLocation()) {
         for (Id tokenId : location->getTokenIds()) {
            nodeIdToParentFileMap.emplace(
               tokenId, std::make_pair(getFileNodeId(location->getFilePath()),
...
StorageEdge PersistentStorage::getEdgeById(Id edgeId) const { return m_sqliteIndexStorage.getEdgeById(edgeId); }

std::shared_ptr<SourceLocationCollection> PersistentStorage::getFullTextSearchLocations(const std::wstring& searchTerm,
                                                                                        bool caseSensitive) const
{
   ...
   MessageStatus(std::wstring(L"Searching fulltext (case-") + (caseSensitive ? L"sensitive" : L"insensitive") + L"): " +
                    searchTerm,
                 false, true)
      .dispatch();

   {
      ...
      for (std::vector<FullTextSearchResult> fileResults : utility::splitToEquallySizedParts(
         ...
         std::shared_ptr<std::thread> thread =
            std::make_shared<std::thread>([this, &searchTerm, &caseSensitive, &codec,
                                           /*no ref here!*/ fileResults, &collection, &collectionMutex]() {
               ...
               for (const FullTextSearchResult& fileResult : fileResults) {
                  ...
                  for (int pos : fileResult.positions) {
...

                     {
                        ...
                        collection->addSourceLocation(LOCATION_FULLTEXT_SEARCH, locationId, std::vector<Id>(), filePath,
   ...
   MessageStatus(std::to_wstring(collection->getSourceLocationCount()) + L" results in " +
...
std::vector<SearchMatch> PersistentStorage::getAutocompletionSymbolMatches(const std::wstring& query,
      ...
      for (const Id& elementId : result.elementIds) {
         if (elementId != 0) {
            ...
            if (!match.hasChildren &&
                acceptedNodeTypes == NodeTypeSet::all()) // TODO: check if node types of children match
            {
...
std::vector<SearchMatch> PersistentStorage::getAutocompletionFileMatches(const std::wstring& query,
                                                                         size_t maxResultsCount) const
{
   const std::vector<SearchResult> results = m_fileIndex.search(
      query, NodeTypeSet::all().getWithMatchingKept([](const NodeType& type) { return type.isFile(); }),
      maxResultsCount, 100);
...
std::vector<SearchMatch> PersistentStorage::getAutocompletionCommandMatches(const std::wstring& query,
                                                                            NodeTypeSet acceptedNodeTypes) const
{
   ...
   for (const SearchResult& result : results) {
      ...
      if (acceptedNodeTypes == NodeTypeSet::all() || (match.getCommandType() == SearchMatch::COMMAND_NODE_FILTER &&
...
std::shared_ptr<Graph> PersistentStorage::getGraphForAll() const
{
   ...
   m_sqliteIndexStorage.forEach<StorageNode>([&, sdk_size](StorageNode&& storageNode) {
      ...
      if (type.isFile()) {
         ...
         if (showNode && (type.isPackage() || !m_hierarchyCache.isChildOfVisibleNodeOrInvisible(storageNode.id))) {
...
std::shared_ptr<Graph> PersistentStorage::getGraphForTrail(Id originId, Id targetId, NodeKindMask nodeTypes,
                                                           Edge::TypeMask edgeTypes, bool nodeNonIndexed, size_t depth,
                                                           bool directed) const
{
   TRACE();
   ...
   while (nodeIdsToProcess.size() && (!depth || currentDepth < depth)) {
      std::vector<StorageEdge> edges = forward ? m_sqliteIndexStorage.getEdgesBySourceIds(nodeIdsToProcess)
                                               : m_sqliteIndexStorage.getEdgesByTargetIds(nodeIdsToProcess);

      if (!directed || edgeTypes & Edge::LAYOUT_VERTICAL) {
         utility::append(edges, forward ? m_sqliteIndexStorage.getEdgesByTargetIds(nodeIdsToProcess)
...
PersistentStorage::getSourceLocationsForTokenIds(const std::vector<Id>& tokenIds) const
{
   TRACE();
   ...
   if (nonFileIds.size()) {
      ...
      for (const StorageSourceLocation& sourceLocation :
         ...
         if (!path.empty()) {
            collection->addSourceLocation(type, sourceLocation.id, {it->second}, path, sourceLocation.startLine,
                                          sourceLocation.startCol, sourceLocation.endLine, sourceLocation.endCol);
         }
      }
   }
...
std::shared_ptr<SourceLocationCollection>
PersistentStorage::getSourceLocationsForLocationIds(const std::vector<Id>& locationIds) const
{
   TRACE();
   ...
   for (StorageSourceLocation location : m_sqliteIndexStorage.getAllByIds<StorageSourceLocation>(locationIds)) {
      ...
      collection->addSourceLocation(intToLocationType(location.type), location.id,
                                    m_locationIdToElementIds[location.id], getFileNodePath(location.fileNodeId),
                                    location.startLine, location.startCol, location.endLine, location.endCol);
   }
...
PersistentStorage::getSourceLocationsForLinesInFile(const FilePath& filePath, size_t startLine, size_t endLine) const
{
   TRACE();
...
std::shared_ptr<SourceLocationFile> PersistentStorage::getSourceLocationsOfTypeInFile(const FilePath& filePath,
...
std::shared_ptr<SourceLocationCollection>
PersistentStorage::getErrorSourceLocations(const std::vector<ErrorInfo>& errors) const
{
   TRACE();
   ...
   for (const ErrorInfo& error : errors) {
      ...
      collection->addSourceLocation(LOCATION_ERROR, locationId, std::vector<Id>(1, error.id), FilePath(error.filePath),
                                    error.lineNumber, error.columnNumber, error.lineNumber, error.columnNumber);
   }
...
Id PersistentStorage::addNodeBookmark(const NodeBookmark& bookmark)
{
   ...
   const Id id = m_sqliteBookmarkStorage
                    .addBookmark(StorageBookmarkData(bookmark.getName(), bookmark.getComment(),
                                                     bookmark.getTimeStamp().toString(), categoryId))
                    .id;
   ...
   for (const Id& edgeId : bookmark.getEdgeIds()) {
      ...
      m_sqliteBookmarkStorage.addBookmarkedEdge(
         StorageBookmarkedEdgeData(id,
                                   ...
                                   storageEdge.type, sourceNodeActive));
   }
   return id;
}
...
void PersistentStorage::updateBookmark(const Id bookmarkId, const std::wstring& name, const std::wstring& comment,
...
void PersistentStorage::removeBookmark(const Id id) { m_sqliteBookmarkStorage.removeBookmark(id); }

void PersistentStorage::removeBookmarkCategory(Id id) { m_sqliteBookmarkStorage.removeBookmarkCategory(id); }
...
std::vector<NodeBookmark> PersistentStorage::getAllNodeBookmarks() const
{
   ...
   for (const StorageBookmark& storageBookmark : m_sqliteBookmarkStorage.getAllBookmarks()) {
      ...
      if (itCategories != bookmarkCategories.end() && itNodeIds != bookmarkIdToBookmarkedNodeIds.end()) {
         NodeBookmark bookmark(storageBookmark.id, storageBookmark.name, storageBookmark.comment,
...
std::vector<EdgeBookmark> PersistentStorage::getAllEdgeBookmarks() const
{
   ...
   for (const StorageBookmark& storageBookmark : m_sqliteBookmarkStorage.getAllBookmarks()) {
      ...
      if (itCategories != bookmarkCategories.end() && itBookmarkedEdges != bookmarkIdToBookmarkedEdges.end()) {
         EdgeBookmark bookmark(storageBookmark.id, storageBookmark.name, storageBookmark.comment,
                               storageBookmark.timestamp,
                               BookmarkCategory(itCategories->second.id, itCategories->second.name));
...
std::vector<BookmarkCategory> PersistentStorage::getAllBookmarkCategories() const
{
   ...
   for (const StorageBookmarkCategory& storageBookmarkCategory : m_sqliteBookmarkStorage.getAllBookmarkCategories()) {
      categories.push_back(BookmarkCategory(storageBookmarkCategory.id, storageBookmarkCategory.name));
   }
   return categories;
}
...
TooltipInfo PersistentStorage::getTooltipInfoForTokenIds(const std::vector<Id>& tokenIds, TooltipOrigin origin) const
{
   ...
   if (origin == TOOLTIP_ORIGIN_CODE) {
      info.offset = math::Vec2i(20, 30);
   } else if (origin == TOOLTIP_ORIGIN_GRAPH) {
      info.offset = math::Vec2i(50, 20);
   }
...
TooltipSnippet PersistentStorage::getTooltipSnippetForNode(const StorageNode& node) const
{
   ...
   if (nameHierarchy.hasSignature()) {
      ...
      while (sigLoc) {
         ...
         std::vector<std::string> lines =
            getFileContent(sigLoc->getFilePath(), false)
               ->getLines(static_cast<unsigned int>(sigLoc->getLineNumber()),
         ...
         std::shared_ptr<SourceLocationFile> file =
            getSourceLocationsForLinesInFile(sigLoc->getFilePath(), sigLoc->getStartLocation()->getLineNumber(),
                                             sigLoc->getEndLocation()->getLineNumber());

         file->forEachStartSourceLocation([&sigLoc, &annotations, &lines](SourceLocation* loc) {
            if ((loc->getType() == LOCATION_TOKEN || loc->getType() == LOCATION_QUALIFIER) && sigLoc->contains(*loc)) {
               ...
               for (size_t i = 0;
                    i < loc->getEndLocation()->getLineNumber() - sigLoc->getLineNumber() && i < lines.size(); i++) {
         ...
         for (const Annotation& annotation : annotations) {
            std::wstring text = code.substr(annotation.startPos + offset, annotation.endPos - annotation.startPos + 1);
         ...
         for (const std::pair<Id, std::wstring>& p : annotatedTexts) {
            ...
            if (pos != std::wstring::npos) {
               ...
               snippet.locationFile->addSourceLocation(loc->getType(), loc->getLocationId(), loc->getTokenIds(), 1,
                                                       pos + 1, 1, pos + p.second.size());
      ...
      snippet.code = utility::breakSignature(nameHierarchy.getSignature().getPrefix(), nameHierarchy.getQualifiedName(),
                                             nameHierarchy.getSignature().getPostfix(), 50,
      ...
      std::set<std::pair<std::wstring, Id>,
      ...
      for (const auto& typeNode : m_sqliteIndexStorage.getAllByIds<StorageNode>(typeNodeIds)) {
         typeNames.insert(
            std::make_pair(NameHierarchy::deserialize(typeNode.serializedName).getQualifiedName(), typeNode.id));
      }
      ...
      for (const auto& p : typeNames) {
         ...
         while (pos != std::wstring::npos) {
            ...
            if (!inRange) {
               snippet.locationFile->addSourceLocation(LOCATION_TOKEN, 0, std::vector<Id>(1, p.second), 1, pos + 1, 1,
                                                       pos + p.first.size());
      ...
      snippet.locationFile->addSourceLocation(LOCATION_TOKEN, 0, std::vector<Id>(1, node.id), 1, 1, 1,
                                              snippet.code.size());
   }
...
TooltipInfo
PersistentStorage::getTooltipInfoForSourceLocationIdsAndLocalSymbolIds(const std::vector<Id>& locationIds,
                                                                       const std::vector<Id>& localSymbolIds) const
{
   ...
   if (!locationIds.empty()) {
      std::wstring fileLanguage =
         getFileNodeLanguage(m_sqliteIndexStorage.getFirstById<StorageSourceLocation>(locationIds.front()).fileNodeId);
      ...
      for (const StorageNode& node : m_sqliteIndexStorage.getAllByIds<StorageNode>(nodeIds)) {
         ...
         snippet.locationFile =
            std::make_shared<SourceLocationFile>(FilePath(L"main.txt"), fileLanguage, true, true, true);
   ...
   for (Id id : localSymbolIds) {
      ...
      snippet.locationFile = std::make_shared<SourceLocationFile>(FilePath(L"main.txt"), L"", true, true, true);
      snippet.locationFile->addSourceLocation(LOCATION_LOCAL_SYMBOL, 0, std::vector<Id>(1, id), 1, 1, 1,
                                              snippet.code.size());

      info.snippets.push_back(snippet);
   }

   info.offset = math::Vec2i(0, 15);
...
std::unordered_map<Id, std::set<Id>> PersistentStorage::getFileIdToImportingFileIdMap() const
{
   std::unordered_map<Id, std::set<Id>> fileIdToImportingFileIdMap;
   {
      ...
      m_sqliteIndexStorage.forEachOfType<StorageEdge>(
         Edge::typeToInt(Edge::EDGE_IMPORT), [&importedElementIds, &elementIdToImportingFileIds](StorageEdge&& edge) {
...
std::set<Id> PersistentStorage::getReferenced(const std::set<Id>& ids,
...
std::set<Id> PersistentStorage::getReferencing(const std::set<Id>& ids,
                                               std::unordered_map<Id, std::set<Id>> idToReferencingIdMap) const
{
...
void PersistentStorage::addFileNodeToGraph(const StorageNode& storageNode, Graph* const graph) const
{
   ...
   Node* node =
      graph->createNode(storageNode.id, NodeType(NODE_FILE), NameHierarchy(filePath.fileName(), NAME_DELIMITER_FILE),
                        indexed ? DEFINITION_EXPLICIT : DEFINITION_NONE);
   node->addComponent(std::make_shared<TokenComponentFilePath>(filePath, complete));
}

void PersistentStorage::addNodeToGraph(const StorageNode& newNode, const NodeType& type, Graph* graph,
...
void PersistentStorage::addNodesWithParentsAndEdgesToGraph(const std::vector<Id>& nodeIds,
...
void PersistentStorage::addBundledEdgesToGraph(Id nodeId, const std::vector<StorageEdge>& edgesToBundle,
                                               Graph* graph) const
{
   TRACE();
   ...
   for (size_t i = 0; i < nodeIdSets.size(); i++) {
      for (const Id nodeId : *nodeIdSets[i]) {
         for (const std::tuple<Id, Id, std::vector<Id>>& edge :
            ...
            Edge* inheritanceEdge = graph->createEdge(inheritanceEdgeId, Edge::EDGE_INHERITANCE,
                                                      graph->getNodeById(sourceId), graph->getNodeById(targetId));
            ...
            [&](const std::vector<StorageFile>& files) {
               for (const StorageFile& file : files) {
                  m_fullTextSearchIndex.addFile(
                     file.id, codec.decode(m_sqliteIndexStorage.getFileContentById(file.id)->getText()));
               }
            },
            part);
         threads.push_back(thread);
...
void PersistentStorage::buildMemberEdgeIdOrderMap()
{
   ...
   m_sqliteIndexStorage.forEachOfType<StorageEdge>(Edge::typeToInt(Edge::EDGE_MEMBER),
   ...
   for (const StorageSourceLocation& location : m_sqliteIndexStorage.getAllByIds<StorageSourceLocation>(locationIds)) {
      ...
      if (path.extension() == L".java") {
         collection.addSourceLocation(intToLocationType(location.type), location.id, std::vector<Id>(),
                                      FilePath(std::to_wstring(location.fileNodeId)), location.startLine,
                                      location.startCol, location.endLine, location.endCol);
      }
   }
...
void PersistentStorage::buildHierarchyCache()
{
   ...
   m_sqliteIndexStorage.forEachByIds<StorageNode>(sourceNodeIds, [&invisibleParentSourceNodeIds](StorageNode&& node) {
   ...
   for (const StorageEdge& edge : memberEdges) {
      ...
      m_hierarchyCache.createConnection(edge.id, edge.sourceNodeId, edge.targetNodeId, sourceIsVisible,
                                        sourceIsImplicit, targetIsImplicit);
   }

   m_sqliteIndexStorage.forEachOfType<StorageEdge>(Edge::typeToInt(Edge::EDGE_INHERITANCE), [this](StorageEdge&& edge) {
