...
TaskBuildIndex::TaskBuildIndex(size_t processCount, std::shared_ptr<StorageProvider> storageProvider,
                               std::shared_ptr<DialogView> dialogView, const std::string& appUUID,
                               bool multiProcessIndexing)
    : m_storageProvider(storageProvider), m_dialogView(dialogView), m_appUUID(appUUID),
      m_multiProcessIndexing(multiProcessIndexing), m_interprocessIndexingStatusManager(appUUID, 0, true),
      m_indexerCommandQueueStopped(false), m_processCount(processCount), m_interrupted(false), m_indexingFileCount(0),
...
void TaskBuildIndex::doEnter(std::shared_ptr<Blackboard> blackboard)
{
   ...
   if (logger) {
      logFilePath = dynamic_cast<FileLogger*>(logger)->getLogFilePath().wstring();
   }
...
void TaskBuildIndex::doExit(std::shared_ptr<Blackboard> blackboard)
{
   ...
   if (!crashedFiles.empty()) {
      ...
      for (const FilePath& path : crashedFiles) {
         ...
         parserClient->recordError(L"The translation unit threw an exception during indexing. Please check if the "
                                   L"source file "
                                   "conforms to the specified language standard and all necessary options are defined "
                                   "within your project "
                                   "setup.",
                                   true, true, path, ParseLocation(fileId, 1, 1));
...
void TaskBuildIndex::runIndexerProcess(int processId, const std::wstring& logFilePath)
{
   ...
   if (!indexerProcessPath.exists()) {
      m_interrupted = true;
      LOG_ERROR(L"Cannot start indexer process because executable is missing at \"" + indexerProcessPath.wstr() +
                L"\"");
      return;
   }
