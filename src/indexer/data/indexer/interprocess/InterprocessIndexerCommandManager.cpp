...
InterprocessIndexerCommandManager::InterprocessIndexerCommandManager(const std::string& instanceUuid, Id processId,
                                                                     bool isOwner)
    : BaseInterprocessDataManager(s_sharedMemoryNamePrefix + instanceUuid, 1048576 /* 1 MB */, instanceUuid, processId,
                                  isOwner)
{
}
   ...
   const std::vector<std::shared_ptr<IndexerCommand>>& indexerCommands)
{
   ...
   while (access.getFreeMemorySize() < size) {
      ...
      LOG_INFO_STREAM(<< "grow memory - est: " << size << " size: " << currentSize
