...
std::unique_ptr<CxxTypeName> CxxTypeNameResolver::getName(const clang::Type* type)
{
   if (type) {
      ...
      case clang::Type::Typedef: {
         ...
         if (declName) {
            return std::make_unique<CxxTypeName>(declName->getName(), std::vector<std::wstring>(),
                                                 declName->getParent());
         }
         break;
      }
      ...
      case clang::Type::Record: {
         std::unique_ptr<CxxDeclName> declName =
            CxxDeclNameResolver(this).getName(type->getAs<clang::TagType>()->getDecl());
         if (declName) {
      ...
      case clang::Type::Builtin: {
         ...
         return std::make_unique<CxxTypeName>(
            utility::decodeFromUtf8(type->getAs<clang::BuiltinType>()->getName(pp).str()), std::vector<std::wstring>());
      }
      case clang::Type::TemplateSpecialization: {
         const clang::TagType* tagType = type->getAs<clang::TagType>(); // remove this case when NameHierarchy is split
                                                                        // into namepart and parameter part
         if (tagType) {
            std::unique_ptr<CxxDeclName> declName = CxxDeclNameResolver(this).getName(tagType->getDecl());
            if (declName) {
               return std::make_unique<CxxTypeName>(declName->getName(), declName->getTemplateParameterNames(),
                                                    declName->getParent());
            }
                ...
                // important, may help: has no underlying decl!
         {
            ...
            if (declName) {
               ...
               resolver.ignoreContextDecl(
                  templateSpecializationType->getTemplateName().getAsTemplateDecl()->getTemplatedDecl());
               ...
               return std::make_unique<CxxTypeName>(declName->getName(), std::move(templateArguments),
                                                    declName->getParent());
            } else {
      ...
      case clang::Type::DependentName: {
         ...
         return std::make_unique<CxxTypeName>(utility::decodeFromUtf8(dependentType->getIdentifier()->getName().str()),
                                              std::vector<std::wstring>(), std::move(specifierName));
      }
      case clang::Type::DependentTemplateSpecialization: {
         ...
         for (unsigned i = 0; i < dependentType->template_arguments().size(); i++) {
            templateArguments.push_back(resolver.getTemplateArgumentName(dependentType->template_arguments()[i]));
         }

         return std::make_unique<CxxTypeName>(utility::decodeFromUtf8(dependentType->getIdentifier()->getName().str()),
                                              std::move(templateArguments), std::move(specifierName));
      }
