...
CxxVerboseAstVisitor::CxxVerboseAstVisitor(clang::ASTContext* context, clang::Preprocessor* preprocessor,
...
bool CxxVerboseAstVisitor::TraverseStmt(clang::Stmt* stmt)
{
   if (stmt) {
      ...
      LOG_INFO_STREAM_BARE(<< "Indexer - " << getIndentString() << stmt->getStmtClassName() << " <"
...
bool CxxVerboseAstVisitor::TraverseTypeLoc(clang::TypeLoc tl)
{
   if (!tl.isNull()) {
      ...
      LOG_INFO_STREAM_BARE(<< "Indexer - " << getIndentString() << typeLocClassToString(tl) << "TypeLoc <"
                           << loc.startLineNumber << ":" << loc.startColumnNumber << ", " << loc.endLineNumber << ":"
                           << loc.endColumnNumber << ">");
      {
         ScopedSwitcher<unsigned int> switcher(m_indentation, m_indentation + 1);
