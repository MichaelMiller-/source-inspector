...
CxxAstVisitor::CxxAstVisitor(clang::ASTContext* astContext, clang::Preprocessor* preprocessor,
    ...
    : m_astContext(astContext), m_preprocessor(preprocessor), m_client(client), m_indexerStateInfo(indexerStateInfo),
      m_canonicalFilePathCache(canonicalFilePathCache), m_contextComponent(this), m_declRefKindComponent(this),
      m_typeRefKindComponent(this), m_implicitCodeComponent(this), m_indexerComponent(this, astContext, client),
...
CanonicalFilePathCache* CxxAstVisitor::getCanonicalFilePathCache() const { return m_canonicalFilePathCache.get(); }
...
bool CxxAstVisitor::shouldVisitTemplateInstantiations() const { return true; }

bool CxxAstVisitor::shouldVisitImplicitCode() const { return m_implicitCodeComponent.shouldVisitImplicitCode(); }
...
bool CxxAstVisitor::VisitTranslationUnitDecl(clang::TranslationUnitDecl* d) { return true; }
...
bool CxxAstVisitor::TraverseCallExpr(clang::CallExpr* s) { return TraverseCallCommon(s); }

bool CxxAstVisitor::TraverseCXXMemberCallExpr(clang::CXXMemberCallExpr* s) { return TraverseCallCommon(s); }

bool CxxAstVisitor::TraverseCXXOperatorCallExpr(clang::CXXOperatorCallExpr* s) { return TraverseCallCommon(s); }
...
{
   return utility::getParseLocation(sourceLocation, m_astContext->getSourceManager(), m_preprocessor,
                                    m_canonicalFilePathCache);
}

ParseLocation CxxAstVisitor::getParseLocation(const clang::SourceRange& sourceRange) const
{
   return utility::getParseLocation(sourceRange, m_astContext->getSourceManager(), m_preprocessor,
                                    m_canonicalFilePathCache);
}
