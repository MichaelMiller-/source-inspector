...
int Edge::typeToInt(EdgeType type) { return type; }
...
Edge::Edge(Id id, EdgeType type, Node* from, Node* to) : Token(id), m_type(type), m_from(from), m_to(to)
{
   m_from->addEdge(this);
   m_to->addEdge(this);
}

Edge::Edge(const Edge& other, Node* from, Node* to) : Token(other), m_type(other.m_type), m_from(from), m_to(to)
{
   m_from->addEdge(this);
   m_to->addEdge(this);
...
Edge::EdgeType Edge::getType() const { return m_type; }

bool Edge::isType(TypeMask mask) const { return (m_type & mask) > 0; }

Node* Edge::getFrom() const { return m_from; }

Node* Edge::getTo() const { return m_to; }
...
bool Edge::isNode() const { return false; }

bool Edge::isEdge() const { return true; }
...
std::wstring Edge::getReadableTypeString() const { return getReadableTypeString(m_type); }
