...
size_t SourceLocationCollection::getSourceLocationFileCount() const { return m_files.size(); }

std::shared_ptr<SourceLocationFile>
SourceLocationCollection::getSourceLocationFileByPath(const FilePath& filePath) const
{
...
SourceLocation* SourceLocationCollection::addSourceLocation(LocationType type, Id locationId, std::vector<Id> tokenIds,
                                                            const FilePath& filePath, size_t startLineNumber,
                                                            size_t startColumnNumber, size_t endLineNumber,
                                                            size_t endColumnNumber)
{
   if (startLineNumber > endLineNumber || (startLineNumber == endLineNumber && startColumnNumber > endColumnNumber)) {
      LOG_ERROR(L"SourceLocation has wrong boundaries: " + filePath.wstr() + L" " + std::to_wstring(startLineNumber) +
                L":" + std::to_wstring(startColumnNumber) + L" " + std::to_wstring(endLineNumber) + L":" +
                std::to_wstring(endColumnNumber));
      return nullptr;
   }
   ...
   return file->addSourceLocation(type, locationId, tokenIds, startLineNumber, startColumnNumber, endLineNumber,
                                  endColumnNumber);
}
...
SourceLocation* SourceLocationCollection::addSourceLocationCopy(const SourceLocation* location)
{
   ...
   SourceLocationFile* file = createSourceLocationFile(location->getFilePath(), other->getLanguage(), other->isWhole(),
                                                       other->isComplete(), other->isIndexed());
...
void SourceLocationCollection::addSourceLocationCopies(const SourceLocationCollection* other)
{
   other->forEachSourceLocationFile(
      [this](std::shared_ptr<SourceLocationFile> otherFile) { addSourceLocationCopies(otherFile.get()); });
}

void SourceLocationCollection::addSourceLocationCopies(const SourceLocationFile* otherFile)
{
   SourceLocationFile* file =
      createSourceLocationFile(otherFile->getFilePath(), otherFile->getLanguage(), otherFile->isWhole(),
                               otherFile->isComplete(), otherFile->isIndexed());
...
SourceLocationFile* SourceLocationCollection::createSourceLocationFile(const FilePath& filePath,
                                                                       const std::wstring& language, bool isWhole,
                                                                       bool isComplete, bool isIndexed)
{
...
{
   ostream << L"Locations:\n";
   base.forEachSourceLocationFile([&ostream](std::shared_ptr<SourceLocationFile> f) { ostream << *(f.get()); });
   return ostream;
}
