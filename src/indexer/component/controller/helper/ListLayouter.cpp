...
void ListLayouter::layoutMultiColumn(math::Vec2i viewSize, std::vector<std::shared_ptr<DummyNode>>* nodes)
{
   size_t colsFinal;
   ...
   size_t nodesPerCol =
      (colsFinal == 1 ? visibleNodes.size()
                      : static_cast<size_t>(std::ceil((visibleNodes.size() + colsFinal - 1) / double(colsFinal))));
...
void ListLayouter::layoutSquare(std::vector<std::shared_ptr<DummyNode>>* nodes, int maxWidth)
{
   ...
   for (int i = cols; i < 100; i++) {
      if (layoutSquareInternal(visibleNodes, math::Vec2i(maxWidth, totalHeight * i / 100), math::Vec2i(gapX, gapY))) {
         auto rect = boundingRect(visibleNodes);
   ...
   layoutSquareInternal(visibleNodes, math::Vec2i(maxWidth, totalHeight * cols / 100), math::Vec2i(gapX, gapY));
}

bool ListLayouter::layoutSquareInternal(std::vector<std::shared_ptr<DummyNode>>& visibleNodes, const math::Vec2i& maxSize,
...
math::Vec4i ListLayouter::boundingRect(const std::vector<std::shared_ptr<DummyNode>>& nodes)
{
   math::Vec4i rect;
...
math::Vec2i ListLayouter::offsetNodes(std::vector<std::shared_ptr<DummyNode>> nodes, int top, int left)
{
   auto rect = boundingRect(nodes);
   math::Vec2i offset(left - rect.x(), top - rect.y());
