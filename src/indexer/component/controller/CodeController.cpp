...
Id CodeController::getSchedulerId() const { return Controller::getTabId(); }
...
void CodeController::handleMessage(MessageActivateLegend* message) { clear(); }
...
void CodeController::handleMessage(MessageCodeShowDefinition* message)
{
   ...
   if (message->inIDE) {
      MessageMoveIDECursor(filePath, static_cast<unsigned int>(lineNumber), static_cast<unsigned int>(columnNumber))
         .dispatch();
      return;
   }
   ...
   showFiles(m_codeParams, CodeScrollParams::toLine(filePath, lineNumber, CodeScrollParams::Target::TOP),
             !message->isReplayed());
}
...
void CodeController::handleMessage(MessageFlushUpdates* message) { showFiles(m_codeParams, m_scrollParams, true); }

void CodeController::handleMessage(MessageFocusIn* message) { getView()->coFocusTokenIds(message->tokenIds); }

void CodeController::handleMessage(MessageFocusOut* message) { getView()->deCoFocusTokenIds(); }

void CodeController::handleMessage(MessageScrollToLine* message)
{
   getView()->scrollTo(CodeScrollParams::toLine(message->filePath, message->line, CodeScrollParams::Target::TOP),
                       false);

   MessageStatus(L"Showing source location: " + message->filePath.wstr() + L" : " + std::to_wstring(message->line))
      .dispatch();
}
...
void CodeController::handleMessage(MessageToNextCodeReference* message)
{
   ...
   std::pair<int, int> referencePos =
      findClosestReferenceIndex(m_references, currentFilePath, currentLineNumber, currentColumnNumber, next);
   std::pair<int, int> localReferencePos =
      findClosestReferenceIndex(m_localReferences, currentFilePath, currentLineNumber, currentColumnNumber, next);
      ...
      if (referenceFileIndex == localReferenceFileIndex) {
         if (referenceFileIndex < 0) {
            ...
            if (m_references[referenceIndex].lineNumber == m_localReferences[localReferenceIndex].lineNumber) {
               if ((next &&
                    m_references[referenceIndex].columnNumber < m_localReferences[localReferenceIndex].columnNumber) ||
                   (!next &&
                    m_references[referenceIndex].columnNumber > m_localReferences[localReferenceIndex].columnNumber)) {
                  localReferenceIndex = -1;
               } else {
...
CodeView* CodeController::getView() const { return Controller::getView<CodeView>(); }
...
std::vector<CodeFileParams> CodeController::getFilesForActiveSourceLocations(const SourceLocationCollection* collection,
                                                                             Id declarationId) const
{
   TRACE();
...
std::vector<CodeFileParams>
CodeController::getFilesForCollection(std::shared_ptr<SourceLocationCollection> collection) const
{
   TRACE();
...
CodeSnippetParams CodeController::getSnippetParamsForWholeFile(std::shared_ptr<SourceLocationFile> locationFile,
                                                               bool useSingleFileCache) const
{
...
std::vector<CodeSnippetParams>
CodeController::getSnippetsForFile(std::shared_ptr<SourceLocationFile> activeSourceLocations) const
{
   TRACE();
   ...
   commentLocations->forEachStartSourceLocation([&](SourceLocation* location) {
      atomicRanges.push_back(SnippetMerger::Range(
         ...
         SnippetMerger::Border(static_cast<int>(location->getOtherLocation()->getLineNumber()), false)));
   });
   ...
   for (const SnippetMerger::Range& range : ranges) {
      ...
      if (params.startLineNumber > 1) {
         const SourceLocation* location = getSourceLocationOfParentScope(params.startLineNumber, scopeLocations.get());
      ...
      for (const std::string& line : textAccess->getLines(static_cast<unsigned int>(params.startLineNumber),
...
std::shared_ptr<SnippetMerger>
CodeController::buildMergerHierarchy(const SourceLocation* location, const SourceLocationFile* scopeLocations,
                                     SnippetMerger& fileScopedMerger,
                                     std::map<int, std::shared_ptr<SnippetMerger>>& mergers) const
{
   std::shared_ptr<SnippetMerger> currentMerger =
      std::make_shared<SnippetMerger>(static_cast<int>(location->getStartLocation()->getLineNumber()),
   ...
   std::map<int, std::shared_ptr<SnippetMerger>>::iterator it =
      mergers.find(static_cast<int>(scopeLocation->getLocationId()));
   if (it == mergers.end()) {
...
const SourceLocation* CodeController::getSourceLocationOfParentScope(size_t lineNumber,
                                                                     const SourceLocationFile* scopeLocations) const
{
   ...
   scopeLocations->forEachStartSourceLocation([&location, lineNumber](SourceLocation* scopeLocation) {
      if (scopeLocation->getLineNumber() < lineNumber &&
          scopeLocation->getEndLocation()->getLineNumber() >= lineNumber && (!location || *location < *scopeLocation)) {
   ...
   for (size_t i = 0; i < lines.size(); i++) {
      ...
      while (pos != std::string::npos) {
         ...
         if (tokenId > 0) {
            ...
            locationFile->addSourceLocation(LOCATION_TOKEN, ++locationId, {tokenId}, startLineNumber + i, posA + 1,
...
std::pair<int, int> CodeController::findClosestReferenceIndex(const std::vector<Reference>& references,
                                                              const FilePath& currentFilePath, size_t currentLineNumber,
                                                              size_t currentColumnNumber, bool next) const
{
...
CodeFileParams* CodeController::addSourceLocations(std::shared_ptr<SourceLocationFile> locationFile)
{
   ...
   setFileState(
      *file, getView()->isInListMode() ? MessageChangeFileView::FILE_SNIPPETS : MessageChangeFileView::FILE_MAXIMIZED,
      m_codeParams.useSingleFileCache);
   return file;
}

void CodeController::setFileState(const FilePath& filePath, MessageChangeFileView::FileState state,
...
void CodeController::setFileState(CodeFileParams& file, MessageChangeFileView::FileState state, bool useSingleFileCache)
{
   ...
   case MessageChangeFileView::FILE_MAXIMIZED:
      if (!file.fileParams) {
         file.fileParams =
            std::make_shared<CodeSnippetParams>(getSnippetParamsForWholeFile(file.locationFile, useSingleFileCache));
      }
...
CodeScrollParams CodeController::firstReferenceScrollParams() const
{
   if (m_references.size()) {
      ...
      return CodeScrollParams::toReference(ref.filePath, ref.locationId, ref.scopeLocationId,
...
void CodeController::showFirstActiveReference(Id tokenId, bool updateView)
{
   ...
   if (m_codeParams.activeLocalSymbolIds.size() == 1 && m_codeParams.activeLocalSymbolIds[0] == tokenId &&
      ...
      collection->forEachSourceLocation([&](SourceLocation* location) {
         ...
         for (Id i : location->getTokenIds()) {
            if (i == tokenId) {
               ...
               if (!firstReference.tokenId ||
                   filePathOrder[location->getFilePath()] < filePathOrder[firstReference.filePath]) {
