...
math::Vec4i GraphController::layoutNestingRecursive(DummyNode* node, int relayoutAccessMaxWidth) const
{
   ...
   if (node->subNodes.size()) {
      if (node->isGroupNode()) {
         auto viewSize = getView()->getViewSize();
...
void GraphController::createLegendGraph()
{
   ...
   auto addText = [this](std::wstring text, int fontSizeDiff, auto position) {
   ...
   auto addNode = [&id, &graph, &nodePositions](NodeKind kind, const std::wstring& name, auto position,
   ...
   // Edges
   {
...

      {
         ...
         y += 20;
         Node* templateFunctionSpecializationNode = addNode(NODE_FUNCTION, L"template_function<ArgumentType>",
                                                            math::Vec2i(x, y + dy * ++i), DEFINITION_IMPLICIT);
