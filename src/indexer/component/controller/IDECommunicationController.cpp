...
bool IDECommunicationController::getEnabled() const { return m_enabled; }

void IDECommunicationController::setEnabled(const bool enabled) { m_enabled = enabled; }
   ...
   const NetworkProtocolHelper::SetActiveTokenMessage& message)
{
   if (message.valid) {
      ...
      if (fileId > 0) {
         ...
         MessageStatus(L"Activating source location from plug-in failed. File " + filePath.wstr() +
...
{
   std::wstring networkMessage =
      NetworkProtocolHelper::buildSetIDECursorMessage(message->filePath, message->row, message->column);

   MessageStatus(L"Jump to source location via plug-in: " + message->filePath.wstr() + L", row: " +
                 std::to_wstring(message->row) + L", col: " + std::to_wstring(message->column))
      .dispatch();
