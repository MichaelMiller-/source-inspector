...
private:
   ...
   void createDummyGraphAndSetActiveAndVisibility(const std::vector<Id>& tokenIds, const std::shared_ptr<Graph> graph,
   ...
   void bundleNodesAndEdgesMatching(std::function<bool(const DummyNode::BundleInfo&, const Node*)> matcher,
                                    size_t count, bool countConnectedNodes, const std::wstring& name);
   ...
   std::shared_ptr<DummyNode> bundleByType(std::list<std::shared_ptr<DummyNode>>& nodes, const NodeType& type,
   ...
   math::Vec4i layoutNestingRecursive(DummyNode* node, int relayoutAccessMaxWidth = -1) const;
   ...
   void relayoutGraph(MessageBase* message, GraphView::GraphParams params, bool withCharacterIndex,
